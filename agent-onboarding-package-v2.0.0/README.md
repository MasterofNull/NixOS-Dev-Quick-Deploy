...
| **AIDB Hooks** | `lib/ai-optimizer.sh` + scripts | Bridge this repo with the private AI-Optimizer stack when present |
...
Run `./scripts/local-ai-starter.sh` whenever you want a clean, public stack that mirrors the private AI-Optimizer layout without depending on that repository. The menu exposes

1. **Prepare shared directories** – creates `~/.local/share/ai-stack/` and `~/.local/share/ai-optimizer/`, runs port checks, and records that the “hand” is ready for the “glove”.
2. **Scaffold the trimmed stack** – copies `templates/local-ai-stack/docker-compose.yml` and `.env.example` into `~/Documents/local-ai-stack/`, wiring Postgres + Timescale, Redis/RedisInsight, and Lemonade Server into a single compose project.
3. **Install tooling** – bootstrap the OpenSkills CLI and MCP templates so local agents can run without the private repo.
4. **Regenerate MCP samples** – optional, but keeps the prompts/docs in sync with whatever skills you layer on.

Once scaffolded, manage the services with standard Compose commands:
...
### AIDB Schema Bootstrap
```bash
# Apply the Timescale/pgvector schema to your Postgres instance
./scripts/bootstrap_aidb_data.sh --dsn postgres://mcp:change_me@localhost:5432/mcp

# Inspect the SQL that gets applied
cat docs/sql/aidb-timeseries-schema.sql
```
...
```bash
# Prepare the trimmed stack (once per machine)
./scripts/local-ai-starter.sh   # choose options 1 + 2

# Launch or update the stack
cd ~/Documents/local-ai-stack
docker compose up -d           # or podman-compose up -d
