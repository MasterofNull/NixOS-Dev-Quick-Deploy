...
- Generic `llm_router.py` shell reduced on `2026-04-16`: free and paid tier execution now delegates through `/control/ai-coordinator/delegate` with bounded profiles instead of logging placeholder fallback or raising `NotImplementedError`
...
- Local fallback status (2026-04-16): bounded local subprocess agents are usable through `/control/agents/spawn`, bounded local teams now execute through `/control/agents/team`, review handoffs now attempt delegated local reviewer execution through `/control/review/agent-handoff`, `/control/runtimes/*` now exposes loopback CLI access plus bounded deploy/rollback verification for allowlisted switchboard-backed lanes, `llm_client.py` can now execute bounded local completions through switchboard's `/v1/chat/completions` ingress, and `llm_router.py` now routes free/paid tiers through the coordinator delegate path instead of placeholder branches. CLI exposure is available via `harness-rpc.js agent-spawn`, `agent-team`, `review-handoff`, `runtime-list`, `runtime-deploy`, and `runtime-rollback` for continued sub-agent work while remote lanes recover. Exact smoke prompts are reliable; broader slice-planning asks may still time out or return empty content.
- Local routing update (2026-04-12): local retrieval/planning prompts now prefer `embedded-assist` instead of the generic `default` lane, so the embedded model is used more directly for low-risk context-heavy work before falling back to chat-heavy local execution.
