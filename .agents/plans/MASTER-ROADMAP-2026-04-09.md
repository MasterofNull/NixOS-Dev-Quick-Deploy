...
- Local fallback status (2026-04-12): bounded local subprocess agents are usable through `/control/agents/spawn`, bounded local teams now execute through `/control/agents/team`, and review handoffs now attempt delegated local reviewer execution through `/control/review/agent-handoff`; CLI exposure is available via `harness-rpc.js agent-spawn`, `agent-team`, and `review-handoff` for continued sub-agent work while remote lanes recover. Exact smoke prompts are reliable; broader slice-planning asks may still time out or return empty content.
