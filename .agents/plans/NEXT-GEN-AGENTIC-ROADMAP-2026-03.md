...
- Deployment and operator telemetry now also has queryable graph views, cross-deployment relatedness reasoning, cluster summaries, root-cluster/failure-family queries, and ranked cause chains, which improves shared context for coordinating future agent slices against deployment history and operational relationships.
