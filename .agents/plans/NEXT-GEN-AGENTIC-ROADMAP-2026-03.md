...
- Deployment and operator telemetry now also has queryable graph views, cross-deployment relatedness reasoning, cluster summaries, root-cluster/failure-family queries, ranked cause chains, and cluster score-breakdown rankings, which improves shared context for coordinating future agent slices against deployment history and operational relationships.
