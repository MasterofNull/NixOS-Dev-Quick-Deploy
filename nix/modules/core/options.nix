      ...
      llamaCpp = {
        ...
        modelCatalog = lib.mkOption {
          ...
          description = ''
            ...
            - qwen3.6-35b-mtp: Qwen3.6 35B A3B MTP Instruct (MTP heads, ~22.5GB Q4_K_XL; needs llama.cpp b9180+)
          ...
          localSystemPrompt = {
            ...
            workflow = lib.mkOption {
              ...
              default = [
                ...
                "For introspection prompts about your operation, memory, orchestration, or limitations, start with aq-feedback-loop --task \"<prompt>\" or aq-context-bootstrap --task \"<prompt>\" --format json; prefer aq-context-manage summary --task \"<prompt>\" --json and embedded-assist as compact search/context helpers, and if they select context-offload, execute sanctioned aq-* preflight or continuation_startup_commands before answering, then gather bounded evidence with get_hints, harness_health, get_working_memory, query_aidb, aq-report, aq-qa 0 --json, or aq-memory search."
    ...
    mcpServers = {
      ...
      repoPath = lib.mkOption {
        ...
        description = ''
          ...
          Configuration split policy (intentional design):
          - Build-time (Nix store, immutable, versioned): safety policy, isolation
            profiles, scheduler policy, AppArmor profiles, OTLP collector config.
            These are generated by pkgs.writeText / pkgs.writeYAML and stored in
            the Nix store. They require a rebuild to change.
          - Runtime (repoPath, mutable, hot-reloadable): intent-routing-map.json,
            workflow-blueprints.json, attention queue (.agents/attention/), and
            drop zone (.agents/drops/). These are read from the live checkout and
            can be updated without a rebuild via POST /control/*/reload endpoints.

          On development systems, repoPath is typically the git working tree
          (e.g., /home/<user>/Documents/NixOS-Dev-Quick-Deploy). Services access
          it via ProtectHome=read-only + explicit ReadWritePaths for mutable
          subdirs, and homeMode=0711 on the primary user home directory provides
          DAC traversal for non-owner service UIDs.

          On production deployments: set repoPath to a path NOT under /home (the
          default /opt/nixos-quick-deploy is correct). Services must not write
          mutable state to the code tree; use dataDir for persistent state instead.
        '';
      };
