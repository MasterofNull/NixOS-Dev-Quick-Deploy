  ...
  grafanaDashboard = builtins.toJSON {
    ...
    panels = [
      ...
      }
      {
        ...
        targets = [
          {
            expr = "100 * amdgpu_vram_used_bytes / clamp_min(amdgpu_vram_total_bytes, 1)";
            refId = "A";
          }
        ];
        gridPos = {
      ...
      }
      {
        ...
        targets = [
          {
            expr = "sum(rate(hybrid_llm_backend_selections_total{backend=\"local\"}[5m]))";
            legendFormat = "local";
            refId = "A";
          }
          {
            expr = "sum(rate(hybrid_llm_backend_selections_total{backend=\"remote\"}[5m]))";
            legendFormat = "remote";
            refId = "B";
          }
        ];
      ...
      # Phase 21.2 — LLM Token Throughput (actual llama.cpp metrics)
      {
        ...
        targets = [
          {
            expr = "sum(rate(llamacpp_tokens_predicted_total[1m]))";
            legendFormat = "generated";
            refId = "A";
          }
          {
            expr = "sum(rate(llamacpp_prompt_tokens_total[1m]))";
            legendFormat = "prompt";
            refId = "B";
          }
        ];
      ...
      # Phase 21.2 — KV Cache Utilization
      {
        ...
        targets = [
          {
            expr = "100 * llamacpp_kv_cache_usage_ratio";
            refId = "A";
          }
        ];
        gridPos = {
      ...
      # Phase 21.2 — Request Latency p95
      {
        ...
        targets = [
          {
            expr = "histogram_quantile(0.95, sum by (le) (rate(llamacpp_request_processing_seconds_bucket[5m])))";
            legendFormat = "p95";
            refId = "A";
          }
          {
            expr = "histogram_quantile(0.50, sum by (le) (rate(llamacpp_request_processing_seconds_bucket[5m])))";
            legendFormat = "p50";
            refId = "B";
          }
        ];
      ...
      # Phase 21.2 — Active Requests / Slot Utilization
      {
        ...
        targets = [
          ...
          {
            expr = "llamacpp_requests_pending";
      ...
      # Phase 21.2 — Embedding Server Throughput
      {
        ...
        targets = [
          {
            expr = "sum(rate(llamacpp_prompt_tokens_total{job=\"llama-cpp-embed\"}[1m]))";
            legendFormat = "tokens";
            refId = "A";
          }
        ];
      ...
      # Phase 21.3 — Embedding Cache Size
      {
        ...
        targets = [
          {
            expr = "embedding_cache_size_keys";
            refId = "A";
          }
        ];
        gridPos = {
      ...
      # Phase 21.3 — Cache Invalidations
      {
        ...
        targets = [
          {
            expr = "sum(increase(embedding_cache_invalidations_total[1h])) by (trigger)";
      ...
      # Phase 5 — Model Reloads
      {
        ...
        targets = [
          {
            expr = "sum(increase(model_reloads_total{status=\"success\"}[24h]))";
            refId = "A";
          }
        ];
        gridPos = {
      ...
      # Phase 5 — Model Reload Duration
      {
        ...
        targets = [
          {
            expr = "histogram_quantile(0.99, rate(model_reload_duration_seconds_bucket[1h]))";
            refId = "A";
          }
        ];
        gridPos = {
