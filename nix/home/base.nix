  ...
  home.activation.createContinueConfig = lib.hm.dag.entryAfter ["writeBoundary"] ''
          ...
          cat > "$_cfg" << 'CONTINUE_EOF'
    {
      ...
      "rules": [
        ...
        "INTROSPECTION MODE: For questions about your operation, capabilities, limitations, memory, orchestration, or collaboration, gather bounded evidence first with harness tools. Prefer get_hints, harness_health, get_working_memory, and query_aidb before shell fallbacks like aq-report --format=json, aq-qa 0 --json, or aq-memory search.",
