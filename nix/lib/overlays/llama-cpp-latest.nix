...
*/
{
  ...
  baseLlamaCpp =
    if enableVulkan
    then prev.llama-cpp.override {vulkanSupport = true;}
    else if enableRocm
    then prev.llama-cpp.override {rocmSupport = true;}
    else if enableCuda
    then prev.llama-cpp.override {cudaSupport = true;}
    else prev.llama-cpp;
in {
  llama-cpp = baseLlamaCpp.overrideAttrs (oldAttrs: {
    ...
    meta =
      ...
      // {
        description =
          (oldAttrs.meta.description or "llama.cpp")
          + " (pinned: ${selected.rev}${
            if useFallback
            then " [fallback]"
            else ""
