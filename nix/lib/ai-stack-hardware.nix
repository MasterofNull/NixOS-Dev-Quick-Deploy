...
in {
  ...
  */
  getEnvironment = {
    ...
    vulkanIcdPath = let
      ...
      icdKey =
        if lib.hasSuffix "aarch64" platform
        then "icd_linux_aarch64"
        else "icd_linux";
    in
  ...
  */
  getTierRecommendations = {tier}: let
  ...
  */
  listSupportedAccelerators = {system}: let
    platform = systemToPlatform system;
