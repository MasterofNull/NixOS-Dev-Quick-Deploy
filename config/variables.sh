...
if [[ -n "${PODMAN_STORAGE_DRIVER+x}" && -n "${PODMAN_STORAGE_DRIVER}" ]]; then
    PODMAN_STORAGE_DRIVER_PRESET=true
else
    PODMAN_STORAGE_DRIVER="$DEFAULT_PODMAN_STORAGE_DRIVER"
    PODMAN_STORAGE_DRIVER_PRESET=false
fi

if [[ -z "${PODMAN_STORAGE_COMMENT:-}" ]]; then
    PODMAN_STORAGE_COMMENT="Using ${PODMAN_STORAGE_DRIVER} driver on detected filesystem."
fi
