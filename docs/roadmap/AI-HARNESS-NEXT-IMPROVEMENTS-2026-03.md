...
- Current boot-stability follow-up: make mutable program state root-owned so tmpfiles does not trip over unsafe ownership transitions, wait for local AIDB health before user-session catalog sync runs, and stop forcing the P14s AMD display-core debug mask while marking TSC unstable on that host to avoid repeated boot-time watchdog warnings
