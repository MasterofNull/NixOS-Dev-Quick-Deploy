...
| 2026-03-13 | deploy QA capability gate realignment | validated_local | `nixos-quick-deploy.sh` now verifies hybrid capability activation through `/workflow/plan` and `/learning/export` plus direct repo-local `aq-qa 0 --json`, avoiding the recursive `/qa/check` timeout path while keeping full Phase 0 available for explicit validation |
