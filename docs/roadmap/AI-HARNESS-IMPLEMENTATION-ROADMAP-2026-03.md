...
Open Risks / Blockers:
- accepted lessons now affect hints, deploy/operator summaries, delegated responses, direct query metadata, direct workflow-plan metadata, direct workflow-tooling-manifest metadata, direct /hints metadata, direct /feedback plus /feedback/evaluate metadata, direct session info/clear metadata, direct discovery capability/token-budget metadata, direct /status plus /stats plus /health plus /health/detailed metadata, direct /search/tree plus memory store/recall plus harness eval/stats/scorecard plus cache stats plus learning stats/export plus model status metadata, direct /context/multi_turn plus /cache/invalidate plus /learning/process plus /learning/ab_compare metadata, direct workflow-run start metadata, persisted workflow-run retrieval, direct workflow mode/event/replay plus workflow isolation metadata, direct workflow session start/get/list plus workflow tree/fork/advance plus workflow blueprints metadata, direct workflow orchestrate/status metadata, direct /review/acceptance metadata, direct ai-coordinator status plus lessons plus skills metadata, direct runtime-control register/list/get/status/deploy/rollback/schedule metadata, and now have dedicated live smoke coverage across those workflow/runtime helper consumers, but broader direct routing/reference materialization remains incomplete beyond those first consumers
