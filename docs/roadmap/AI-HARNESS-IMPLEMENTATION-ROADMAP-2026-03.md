...
| --- | --- | --- | --- |
| Flagship agent CLI coverage | in_progress | Continue CLI packaging and HM activation are now fixed again, repo-native help smokes prove `cn`, `codex`, `qwen`, `gemini`, and `pi` are present on this host, and deploy-time capability verification now reuses direct `aq-qa 0` output instead of timing out on recursive service-backed `/qa/check`; Codex/Qwen/Gemini/Claude CLI delivery is still mixed between declarative, external, and scaffolded | keep support matrix current and package or explicitly classify each remaining surface |
...
| --- | --- | --- | --- |
| 2026-03-13 | command-center dashboard PRSI + harness remediation | validated_live | live `dashboard.html` now renders grouped human-readable PRSI proposals, exposes exact execution/maintenance error detail, fixes PRSI writable state, and the harness maintenance actions for phase-plan, research sync, acceptance checks, and improvement pass all complete successfully on the deployed system |
| 2026-03-13 | deploy QA capability gate realignment | validated_live | `nixos-quick-deploy.sh` now verifies hybrid capability activation through `/workflow/plan` and `/learning/export` plus direct repo-local `aq-qa 0 --json`, avoiding the recursive `/qa/check` timeout path while keeping full Phase 0 available for explicit validation; full quick-deploy now completes again with repo-backed capability verification passing and `aq-qa 0` at `36 pass / 0 fail` |
...
| 2026-03-13 | continuous-learning proposal serialization fix | validated_local | fixed continuous-learning proposal JSONL persistence so optimization proposals serialize `created_at` cleanly instead of spamming `proposal_record_failed` when telemetry-derived proposals are generated |
| 2026-03-13 | continuous-learning embedding path fix | validated_local | fixed continuous-learning pattern embedding fetches to use the OpenAI-compatible `/v1/embeddings` path and parse both legacy-list and modern `data[]` responses so telemetry learning can keep indexing patterns instead of failing on the old `/embed` path |
...
Track Status: `in_progress`
Last Updated: `2026-03-13`
Current Slice: `all remote delegation lanes now use profile-specific compact completion rules, remote-tool-calling keeps the bounded post-tool finalization pass, remote-reasoning now has a bounded reasoning-finalization pass for reasoning-only empty replies, delegated prompt-failure history is now materialized across 1h/24h/7d windows in aq-report and deploy summaries, the delegation contract now adds task-shape-specific rules for bugfix, deploy-safe, review, and research asks, a dedicated multi-lane live smoke proves bounded remote-free, remote-coding, and remote-reasoning delegation either succeed directly or settle into explicit remote-free fallback with a final non-failure contract, and delegated failure windows are being tightened into explicit improving/steady/worsening trend classification with remediation actions; the next gap is validating that those trend signals move in the right direction on live delegated workloads instead of only existing as report structure`
Next Validation:
...
- delegated prompt-contract failures are now recorded, task-shape-specific prompt rules now exist for several common delegated asks, live multi-lane bounded delegation now has explicit success-or-fallback smoke coverage, and failure windows now classify improving/steady/worsening trend state, but the next step is still to prove those revisions actually reduce repeated failure classes over time on live delegated work
...
Track Status: `in_progress`
Last Updated: `2026-03-13`
Current Slice: `accepted lessons now feed back into the hint engine, deploy summary, delegated response metadata, direct query response metadata, direct augment-query response metadata, direct workflow-plan response metadata, direct workflow-tooling-manifest response metadata, direct /hints plus /hints/feedback response metadata, direct /feedback plus /feedback/evaluate response metadata, direct session info/clear response metadata, direct discovery capability/token-budget response metadata, direct /status plus /stats plus /health plus /health/detailed response metadata, direct /search/tree plus memory store/recall plus harness eval/stats/scorecard plus cache stats plus learning stats/export plus model status response metadata, direct /context/multi_turn plus /cache/invalidate plus /learning/process plus /learning/ab_compare metadata, direct /research/web/fetch plus /research/web/browser-fetch plus /research/workflows/curated-fetch metadata, direct /workflow/run/start response metadata, persisted /workflow/run/{session_id} retrieval, direct workflow mode/event/replay plus workflow isolation response metadata, direct /workflow/session/start plus workflow session get/list plus workflow tree/fork/advance plus /workflow/blueprints response metadata, direct /workflow/orchestrate plus /workflow/orchestrate/{task_id} response metadata, direct /review/acceptance response metadata, direct /control/ai-coordinator/status plus /control/ai-coordinator/lessons plus /control/ai-coordinator/skills response metadata, direct runtime-control register/list/get/status/deploy/rollback/schedule response metadata, and now have dedicated live smoke coverage across those workflow/runtime helper consumers from the durable registry; the next gap is broader direct routing/reference materialization beyond those first runtime consumers, with `reload-model` and proposal-apply still needing bounded parity treatment that does not destabilize live runtime validation`
Next Validation:
...
- `scripts/testing/smoke-feedback-lesson-refs.sh`
- `scripts/testing/smoke-session-lesson-refs.sh`
- `scripts/testing/smoke-discovery-lesson-refs.sh`
- `scripts/testing/smoke-workflow-start-lesson-refs.sh`
- `scripts/testing/smoke-workflow-run-lesson-refs.sh`
- `scripts/testing/smoke-workflow-isolation-lesson-refs.sh`
- `scripts/testing/smoke-workflow-runtime-lesson-refs.sh`
- `scripts/testing/smoke-workflow-tree-lesson-refs.sh`
- `scripts/testing/smoke-status-runtime-lesson-refs.sh`
- `scripts/testing/smoke-workflow-orchestrate-lesson-refs.sh`
- `scripts/testing/smoke-health-memory-learning-lesson-refs.sh`
- `scripts/testing/smoke-context-cache-learning-ops-lesson-refs.sh`
- `scripts/testing/smoke-augment-research-lesson-refs.sh`
...
Open Risks / Blockers:
- accepted lessons now affect hints, deploy/operator summaries, delegated responses, direct query metadata, direct augment-query metadata, direct workflow-plan metadata, direct workflow-tooling-manifest metadata, direct /hints plus /hints/feedback metadata, direct /feedback plus /feedback/evaluate metadata, direct session info/clear metadata, direct discovery capability/token-budget metadata, direct /status plus /stats plus /health plus /health/detailed metadata, direct /search/tree plus memory store/recall plus harness eval/stats/scorecard plus cache stats plus learning stats/export plus model status metadata, direct /context/multi_turn plus /cache/invalidate plus /learning/process plus /learning/ab_compare metadata, direct /research/web/fetch plus /research/web/browser-fetch plus /research/workflows/curated-fetch metadata, direct workflow-run start metadata, persisted workflow-run retrieval, direct workflow mode/event/replay plus workflow isolation metadata, direct workflow session start/get/list plus workflow tree/fork/advance plus workflow blueprints metadata, direct workflow orchestrate/status metadata, direct /review/acceptance metadata, direct ai-coordinator status plus lessons plus skills metadata, direct runtime-control register/list/get/status/deploy/rollback/schedule metadata, and now have dedicated live smoke coverage across those workflow/runtime helper consumers, but broader direct routing/reference materialization remains incomplete beyond those first consumers
- `reload-model` and proposal-apply now carry lesson refs in the response contract, but live smoke coverage remains intentionally bounded because those endpoints restart services or apply changes rather than behaving like read-mostly metadata helpers
- the Ralph workflow/orchestrate helper is still an external dependency surface, so its lesson-ref smoke should remain bounded and metadata-focused rather than being treated as a blocker for unrelated local coordinator paths
...
Track Status: `in_progress`
Last Updated: `2026-03-13`
Current Slice: `workflow runs now persist live orchestrator policy for top-level callers, delegated runtime responses surface normalized caller identity, delegated failure telemetry records requester-role and handoff metadata, `/query` returns compact orchestration metadata while propagating requester-role fields into internal autorun audit rows, aq-report now summarizes reviewer state by requester role, review type, reviewed agent, reviewed profile, blueprint, and task class, the live workflow review smoke proves Continue/editor rescue acceptance, repo-refactor patch-review classification, bounded research artifact-review persistence, remote-reasoning plan-review persistence, deploy-safe-ops artifact-review persistence, coding-bugfix acceptance persistence, nixos-service-hardening artifact-review persistence, and PRSI self-improvement cycle-report persistence end to end, and a direct query smoke now proves bugfix, Continue/editor rescue, patch-review, source-bounded research, deploy-safe, NixOS-hardening, PRSI self-improvement, shared-skill registry, and delegation-contract task classes preserve orchestration, prompt coaching, and lesson refs; the next gap is broader query/editor acceptance coverage across still more task classes`
Next Validation:
...
- `scripts/testing/smoke-workflow-review-contract.sh`
- `scripts/testing/smoke-query-task-classes.sh`
Open Risks / Blockers:
- top-level orchestrator identity is now explicit in workflow state, delegated responses, direct query responses, and workflow review summaries, and live acceptance now covers continue-editor rescue, repo-refactor, bounded research, remote-reasoning, deploy-safe-ops, coding-bugfix, nixos-service-hardening, PRSI self-improvement, and direct-query bugfix, continue/editor rescue, patch-review, source-bounded research, deploy-safe, hardening, PRSI, shared-skill registry, plus delegation-contract task shapes, but broader coverage across more query shapes is still unfinished
...
- `python3 scripts/testing/test-workflow-review-gate.py`
- `python3 scripts/ai/aq-report --format json | jq '.intent_contract_compliance | {accepted_by_reviewed_profile, rejected_by_reviewed_profile}'`
...
Track Status: in_progress
Last Updated: 2026-03-13
Current Slice: `compact prompt-coaching hints now teach route selection across local, remote-free, remote-coding, remote-tool-calling, and Continue/editor rescue lanes, and now also cover patch-review, bounded research/RAG workflows, deploy-safe ops, bugfix/debug asks, NixOS service-hardening asks, PRSI/self-improvement asks, shared-skill registry asks, and delegation/coordinator contract asks; the next gap is extending the same compact guidance to still more task classes without inflating default hint payloads`
Next Validation: `scripts/ai/aq-hints "route local vs remote coding lane" --format json`, `scripts/ai/aq-hints "review this patch safely" --format json`, `scripts/ai/aq-hints "deploy this service safely with rollback" --format json`, `scripts/ai/aq-hints "debug this failing regression safely" --format json`, `scripts/ai/aq-hints "harden this nixos service declaratively" --format json`, `scripts/ai/aq-hints "run one pessimistic self-improvement cycle" --format json`, `scripts/ai/aq-hints "sync this approved skill source into the shared skill registry" --format json`, `scripts/ai/aq-hints "delegate this bounded task through the coordinator" --format json`, and live `/hints?q=continue+editor+rescue`
Open Risks / Blockers:
- route-selection, patch-review, research, deploy-safe, bugfix, service-hardening, PRSI/self-improvement, skill-registry, and delegation/coordinator guidance are now explicit in hints, but broader task-class quick references still need the same compact treatment
...
Tasks:
1. teach progressive-disclosure prompt shapes by more task classes
