...
| Continuous quality benchmarking | Common use of repeatable eval/regression loops | **Near parity** — Phase 43: `harness-gap-eval-pack.json` expanded to 12 SWE-grade cases; `publish-eval-trend.py` reads scores.sqlite and emits eval-trend.json (delta, trend label, regression flag); `run-benchmark-gate.sh` integrates pack + trend + threshold enforcement; aq-qa `0.9.8` validates | `Near parity` (Phase 43) |
...
| Multi-surface client adapters (IDE/CLI bridge quality) | `sweep`, `cody-public-snapshot`, `tabby` show strong IDE-centric integration | **Near parity** — Phase 50: `smoke-ide-adapter-compat.sh` covers Continue config wiring, CLI reachability (claude/codex/gemini/aider/aqd), coordinator HTTP surface from IDE perspective, VSCodium extension install checks, MCP adapter config; aq-qa `0.9.15` validates | `Near parity` (Phase 50) |
...
| Dynamic VM-level Sandboxing | `exe.dev` throwaway sandboxes | **Planned — unassigned sandbox program** | `P0` |
| Best-of-N Parallel Runner | Multi-sandbox parallel SDLC run | **Planned — unassigned sandbox program** | `P0` |
| Credential & Cost Scoping | Scoped sub-keys with dollar budgets | **Planned — unassigned sandbox program** | `P1` |
