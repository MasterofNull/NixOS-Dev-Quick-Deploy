...
- Harness-first workflow operationalization (Phase 6 process discipline):
  ...
  - Added static CI/local gates:
    ...
    - `scripts/testing/check-harness-first-pr-evidence-gate.sh` (PR high-impact path enforcement)
  ...
  - Added reviewer enforcement for harness policy surface:
    ...
    - `scripts/testing/check-harness-first-platform-owner-approval.py` (requires platform-owner approval on PRs changing that file)
  - Wired harness-first gates into:
    - `.github/workflows/test.yml` (`Syntax Validation` job)
    - `scripts/automation/run-advanced-parity-suite.sh` (blocking)
...
- Intent-contract adoption improvement (Phase 2/4 support):
  ...
  - `scripts/testing/smoke-focused-parity.sh` workflow run smoke now includes `intent_contract`
- PRSI orchestrator control plane implemented:
  - `scripts/automation/prsi-orchestrator.py` (sync/list/approve/reject/execute/cycle)
  - Queue state and action logs persisted under PRSI paths
- Optimizer executor integration:
  - `scripts/ai/aq-optimizer` supports `--actions-json` and `--output-json`
...
- Reporting expansion (`scripts/ai/aq-report`):
...
- Intent-contract defaulting expansion (Phase 2/4 support):
  - `scripts/ai/harness-rpc.js run-start` now auto-injects a default valid `intent_contract`
...
- Eval stability hardening (Phase 4.1):
  - `scripts/automation/run-gap-eval-pack.py` now skips non-evaluable cases (no retrieval evidence) instead of counting them as hard failures
...
- CI regression guard for report schema (Phase 6.2 support):
  - Added `scripts/testing/check-aq-report-contract.sh` to validate `aq-report --format=json` output contract
...
- Prewarm strategy improvement (Phase 3.1):
  - `scripts/data/seed-routing-traffic.sh` now supports replay passes (`--replay`) and optional dynamic gap-driven query seeding from PostgreSQL
...
- Intent-contract coverage hardening at workflow ingress (Phase 2/4 support):
  ...
  - Validation tooling remains green: `scripts/testing/check-aq-report-contract.sh` and `scripts/testing/check-aq-report-runtime-sections.sh`
...
- Budget-aware PRSI policy execution (Phase 4/6 integration):
  ...
  - Upgraded `scripts/automation/prsi-orchestrator.py` to enforce:
  ...
  - Added declarative wiring for runtime policy/state paths in Nix service env:
    - `PRSI_POLICY_FILE`
    - `PRSI_STATE_PATH`
  - Extended `scripts/testing/validate-runtime-declarative.sh` to validate PRSI policy file + env wiring
...
- Hint diversity KPI + report section (Phase 2.1 + PRSI loop quality):
  - Added `hint_diversity` metrics to `scripts/ai/aq-report` JSON contract:
  ...
  - Extended `scripts/testing/check-aq-report-contract.sh` to enforce `hint_diversity` schema presence
...
- Hint feedback database + semantic profile loop (Phase 2.2 + PRSI learning):
  - Added `scripts/data/sync-hint-feedback-db.py` to ingest `hint-feedback.jsonl` into Postgres and build semantic aggregate profiles
  - New Postgres tables:
...
- Auto-remediation for low intent-contract coverage + stale gap curation (Phase 2/6):
  ...
  - Extended `scripts/testing/validate-runtime-declarative.sh` to enforce remediator option/env/script wiring
- Phase 1/3 observability + routing closure slice (2026-03-04):
  ...
  - Added `scripts/data/seed-tool-audit-traffic.sh` to exercise diverse hybrid tool endpoints for telemetry breadth
  - Added `scripts/testing/check-aq-report-metric-smoke.sh` and wired it into CI + advanced parity suite
  - Added `scripts/testing/check-routing-fallback.sh` and wired it into advanced parity suite
  - Hardened `scripts/testing/validate-ai-slo-runtime.sh` with API-key auth and curl timeouts
- Phase 4 eval-loop closure slice (2026-03-04):
  - Fixed fallback scoring bug in `scripts/automation/run-gap-eval-pack.py` (string-form hint handling)
  ...
  - Fixed `scripts/ai/aq-report --aidb-import` ingestion path to modern AIDB `/documents` with legacy fallback and API-key support
- Post-deploy verification pass (2026-03-04, latest):
  - Completed runtime checks:
    - `scripts/testing/validate-ai-slo-runtime.sh` (schema + queue-depth PASS; switchboard check still intermittently skipped)
...
- PRSI research corpus indexing + AIDB ingestion:
  ...
  - Synced corpus sources into AIDB via `scripts/data/sync-knowledge-sources`
...
- Phase 7.1 executable gate implementation:
  ...
  - Added contract validator: `scripts/testing/check-prsi-cycle-contract.sh`
  - Wired CI gate in `.github/workflows/test.yml` (`Syntax Validation` job)
  - Wired local release gate in `scripts/automation/run-advanced-parity-suite.sh` (blocking)
  - Added PRSI bootstrap integration validator: `scripts/testing/check-prsi-bootstrap-integrity.sh`
  - Wired PRSI bootstrap integrity gate in CI (`Syntax Validation` job)
  - Added dynamic PRSI discovery dry-run slice: `scripts/automation/run-prsi-discovery-slice.sh`
  - Wired discovery slice gate into `scripts/automation/run-advanced-parity-suite.sh` (blocking)
  - Added CI-safe PRSI static gate bundle: `scripts/testing/check-prsi-phase7-static-gates.sh`
...
- Tool: `scripts/ai/aq-knowledge-import.sh "NixOS module options basics"`
...
- Tool: `scripts/ai/aq-knowledge-import.sh "tc3 feedback validation after patch"`
...
- Tool: `scripts/ai/aq-knowledge-import.sh "Explain lib.mkIf and lib.mkForce in NixOS modules in 3 bullet points"`
...
Tasks:
1. Baseline snapshot
- Tools: `scripts/testing/check-mcp-health.sh`, `scripts/ai/aq-report --since=7d --format=text`
...
- Tools: `scripts/data/seed-routing-traffic.sh`, `scripts/ai/aq-report`
...
- `scripts/ai/aq-report --since=7d --format=json` → routing present (`local=24`, `remote=4`), cache present (`77.5%`)
...
- Tools: `scripts/ai/aq-report`, Prometheus queries
- Success criteria: routing/cache sections remain populated across restarts and metric variant names.

3. CI metric smoke
- Tools: `.github/workflows/test.yml`, `scripts/data/seed-routing-traffic.sh`, `scripts/ai/aq-report`
- Success criteria: CI fails if sections 2/3/9 are unavailable after seeded traffic.

Current closure state (2026-03-04):
- Implemented:
  - Added runtime/CI metric smoke script: `scripts/testing/check-aq-report-metric-smoke.sh`
  - Wired metric smoke into CI (`.github/workflows/test.yml`) and advanced parity suite
  - Added hybrid endpoint tool-coverage seeding script: `scripts/data/seed-tool-audit-traffic.sh`
...
- Closure evidence update (2026-03-04 pass):
  - `scripts/testing/check-aq-report-metric-smoke.sh` → PASS
...
- Implemented/validated:
  - Added bounded intent remediation loop: `scripts/automation/run-intent-remediation-bounded.sh`
  - Added bounded hint-adoption remediation loop: `scripts/automation/run-hint-adoption-remediation-bounded.sh`
...
- Tools: `scripts/data/seed-routing-traffic.sh`, timer config in Nix modules
- Success criteria: cache hit rate `>= 60%` sustained.

2. Declarative routing SLO policy
- Tools: `nix/modules/core/options.nix`, `scripts/testing/validate-ai-slo-runtime.sh`
...
- Tools: `scripts/testing/check-mcp-health.sh`, `/query` synthetic probes
...
- Implemented/validated:
  ...
  - Added explicit fallback validation script: `scripts/testing/check-routing-fallback.sh` (PASS)
  - Runtime SLO validation hardened with auth + curl timeouts (`scripts/testing/validate-ai-slo-runtime.sh`)
...
- Closure verification evidence (2026-03-04 latest):
  - `scripts/testing/check-routing-fallback.sh` → PASS
  - `scripts/testing/validate-ai-slo-runtime.sh` → PASS
  - `scripts/testing/smoke-agent-harness-parity.sh` → PASS (model detection + profile checks + workflow parity endpoints)
...
- Tools: `scripts/ai/aq-prompt-eval`, `scripts/automation/run-eval.sh --strategy ...`
...
- Tools: `scripts/ai/aq-report --aidb-import`, timers
- Success criteria: weekly report artifacts present and searchable.

Closure evidence (2026-03-04):
- `gap_pack_v1` repaired from persistent `0%`:
  - Fixed fallback evaluator in `scripts/automation/run-gap-eval-pack.py` to score string-based runtime hints
...
- Weekly import/search path validated:
  - Fixed `scripts/ai/aq-report` AIDB import path (`/documents` primary, `/import` legacy fallback)
  - `scripts/ai/aq-report --aidb-import` now succeeds (`Report imported to AIDB`)
...
- Tools: `scripts/testing/check-api-auth-hardening.sh`, targeted tool-policy tests
- Success criteria: unsafe tool metadata/params are blocked and logged with reason.

Closure evidence (2026-03-04):
- `python3 scripts/testing/test-tool-security-auditor.py` → PASS
- `scripts/testing/check-api-auth-hardening.sh` → PASS (`401` enforced on protected runtime path)
...
- Tools: `scripts/automation/prsi-orchestrator.py`, `scripts/ai/aq-optimizer`
- Success criteria: each execute cycle writes all required artifacts listed in policy.

2. Add artifact contract check to CI.
- Tools: `.github/workflows/test.yml`, new `scripts/testing/check-prsi-cycle-contract.sh`
...
Closure evidence:
- `scripts/testing/check-prsi-cycle-contract.sh` (schema + artifact contract + cycle-id consistency)
...
- Tools: `scripts/testing/validate-runtime-declarative.sh`, `scripts/testing/check-aq-report-contract.sh`, `scripts/testing/check-aq-report-runtime-sections.sh`
...
- Tools: `scripts/testing/chaos-harness-smoke.sh`, `scripts/testing/check-api-auth-hardening.sh`
...
- Tools: `scripts/testing/smoke-focused-parity.sh`, `scripts/testing/smoke-agent-harness-parity.sh`
- Success criteria: unchanged core workflows stay green after each mutating cycle.

Closure evidence:
- `config/prsi/validation-matrix.json`
- `scripts/testing/check-prsi-validation-matrix.sh`
- `scripts/testing/check-prsi-phase7-program.sh` runs auth + chaos + focused + agent parity scans as blocking gate
...
- Tools: `scripts/ai/aq-report`, `scripts/automation/prsi-orchestrator.py`
...
- Tools: `scripts/automation/run-harness-regression-gate.sh`
...
- Integrity/contamination gate: `scripts/automation/run-prsi-eval-integrity-gate.sh` (fails on holdout overlap)
- Negative controls: `data/prsi-negative-control-canary.json` + `scripts/automation/run-prsi-canary-suite.sh`
- Replay reproducibility baseline gate: `scripts/automation/run-harness-regression-gate.sh --offline`
- Eval/version pinning gate: `scripts/testing/check-prsi-eval-pinning.sh` + `config/prsi/eval-pinning-policy.json`
...
- Tools: `scripts/testing/chaos-harness-smoke.sh`, systemd timers
...
- Tools: `scripts/ai/aq-report`, Prometheus metrics
...
- Fault-injection gate: `scripts/testing/chaos-harness-smoke.sh` (blocking in Phase 7 program gate)
- SLO runtime checks: `scripts/testing/validate-ai-slo-runtime.sh` (blocking in advanced parity suite)
...
- Tools: `scripts/automation/prsi-orchestrator.py cycle`, service logs
...
- Independent verifier lane enforcement for high risk:
  ...
  - `scripts/automation/prsi-orchestrator.py verify` + execution skip if verifier missing
- Stop-condition drill gate: `scripts/automation/run-prsi-stop-condition-drill.sh`
- Budget discipline gate: `scripts/testing/check-prsi-budget-discipline.sh`
...
Status: `completed`
Evidence: `config/schemas/prsi/*.schema.json`, `scripts/testing/check-prsi-cycle-contract.sh`, CI + advanced parity wiring.
...
Status: `completed`
Evidence: `config/prsi/confidence-calibration-policy.json` + `scripts/testing/check-prsi-confidence-calibration.sh` (PASS; latest ece=0.1057).

3. No explicit negative-control/canary suite.
- Add known non-improvement actions to verify scorer rejects false gains.
Status: `completed`
Evidence: `data/prsi-negative-control-canary.json` + `scripts/automation/run-prsi-canary-suite.sh`.
...
Status: `completed`
Evidence: `config/prsi/eval-pinning-policy.json` + `scripts/testing/check-prsi-eval-pinning.sh` (PASS; required SHA256 pins enforced).

5. No formal “quarantine to remediation” workflow.
- Add task template and SLA for quarantined actions.
Status: `completed`
Evidence: `config/prsi/quarantine-workflow.json`, `docs/prsi/PRSI-QUARANTINE-RUNBOOK.md`, `data/prsi-artifacts/quarantine-template.json`, `scripts/testing/check-prsi-quarantine-workflow.sh`.
...
Status: `completed`
Evidence: `config/prsi/high-risk-approval-rubric.json`, `docs/prsi/PRSI-HIGH-RISK-APPROVAL-RUBRIC.md`, `data/prsi-artifacts/high-risk-approval-template.json`, `scripts/testing/check-prsi-high-risk-approval-rubric.sh`.
...
- Completed: declarative post-deploy KPI maintenance wiring for bounded remediation loops.
  ...
  - Extended `scripts/testing/validate-runtime-declarative.sh` to enforce options/env/script wiring.
- Completed: gated phase closeout verification.
  - `scripts/testing/check-prsi-phase7-program.sh` → PASS end-to-end
  - `scripts/testing/validate-ai-slo-runtime.sh` → PASS
  - `scripts/testing/check-routing-fallback.sh` → PASS
- Completed: bounded KPI remediation loops for Phase 2 quality targets.
  - Added `scripts/automation/run-intent-remediation-bounded.sh` to iteratively run capped intent remediation until coverage target is met or budget is exhausted.
  - Added `scripts/automation/run-hint-adoption-remediation-bounded.sh` to run capped hint-injected mutation probes until adoption target is met or budget is exhausted.
  - Latest runtime evidence (`scripts/ai/aq-report --since=7d --format=json`):
...
- Completed: stabilized routing fallback check to avoid false negatives by trying deterministic remote evidence paths before fail.
  - Evidence: `scripts/testing/check-routing-fallback.sh` now passes with healthy local-first runtime.
- Completed: moved generated runtime artifacts out of repository defaults.
  - `scripts/automation/run-gap-eval-pack.py` now defaults `scores.sqlite` to `~/.local/share/nixos-ai-stack/eval/results/scores.sqlite`.
  - `scripts/data/generate-harness-sdk-provenance.sh` now defaults output to `${XDG_STATE_HOME:-$HOME/.local/state}/nixos-ai-stack/harness-sdk-provenance`.
...
- Completed: intent-contract auto-remediation auth + response handling.
  - `scripts/ai/aq-auto-remediate.py` now uses default hybrid key path and accepts modern `/workflow/run/start` response shapes.
- Completed: PRSI confidence calibration gating bootstrap.
  - Added `scripts/data/bootstrap-prsi-confidence-samples.sh`.
  - `scripts/testing/check-prsi-confidence-calibration.sh` now passes with deterministic local bootstrap samples.
- Completed_or_gated: Phase 7 program gate runtime hardening.
  - `scripts/testing/check-prsi-phase7-program.sh` now gates `smoke-agent-harness-parity` on switchboard availability, with strict mode toggle.
