...
- [ ] **6.5.1** Write a test fixture that starts the hybrid-coordinator in a test mode and sends well-formed MCP tool calls, asserting response shapes.
  *Success metric: `pytest tests/integration/test_mcp_contracts.py` passes for all registered MCP tools.*

- [ ] **6.5.2** Add the contract tests to the `Makefile` test target so they run with `make test`.
