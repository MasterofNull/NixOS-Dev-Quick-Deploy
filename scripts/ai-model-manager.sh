...
list_models() {
    ...
    if [[ "$category" == "all" ]]; then
        for cat_dir in "$PROFILES_DIR"/*; do
            if [[ -d "$cat_dir" ]]; then
                ...
                shopt -s nullglob
                for profile in "$cat_dir"/*.yaml "$cat_dir"/*.yml; do
        ...
        if [[ -d "$PROFILES_DIR/$category" ]]; then
            ...
            shopt -s nullglob
            for profile in "$PROFILES_DIR/$category"/*.yaml "$PROFILES_DIR/$category"/*.yml; do
                list_profile_summary "$profile" "$filter"
            done
            shopt -u nullglob
        else
    ...
    if [[ $custom_count -gt 0 ]]; then
        ...
        shopt -s nullglob
        for profile in "$CUSTOM_DIR"/*.yaml "$CUSTOM_DIR"/*.yml; do
            list_profile_summary "$profile" "$filter"
        done
        shopt -u nullglob
    fi
...
list_profile_summary() {
    ...
    local name=$(yq '.name' "$profile" 2>/dev/null || echo "unknown")
    local desc=$(yq '.description' "$profile" 2>/dev/null || echo "No description")
    local runtime=$(yq '.container.runtime' "$profile" 2>/dev/null || echo "unknown")
    local tags=$(yq '.tags[]' "$profile" 2>/dev/null | tr '\n' ' ' || echo "")
...
show_status() {
    ...
    if [[ -d "$ACTIVE_DIR" ]]; then
        shopt -s nullglob
        for link in "$ACTIVE_DIR"/*.yaml "$ACTIVE_DIR"/*.yml; do
            [[ -L "$link" ]] || continue
            found_active=true
            ...
            local model_name=$(yq '.name' "$link" 2>/dev/null || echo "Unknown")
            local runtime=$(yq '.container.runtime' "$link" 2>/dev/null || echo "unknown")
            local model_id=$(yq '.source.model_id' "$link" 2>/dev/null || echo "N/A")
            ...
            if systemctl --user is-active "podman-${service}.service" &>/dev/null 2>&1; then
                local port=$(yq '.port // 8080' "$link" 2>/dev/null)
...
switch_model() {
    ...
    local model_name=$(yq '.name' "$full_path" 2>/dev/null || echo "unknown")
...
show_info() {
    ...
    yq '.' "$full_path" 2>/dev/null || cat "$full_path"
}
