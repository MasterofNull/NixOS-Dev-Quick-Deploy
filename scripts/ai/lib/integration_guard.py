...
def validate_literal_paths(repo: Path | str, paths: Sequence[str]) -> list[str]:
    root = repository_root(repo)
    if not paths:
    ...
    for raw in paths:
        ...
        if not (candidate.is_file() or candidate.is_symlink()):
            tracked = _git(root, "--literal-pathspecs", "ls-files", "--error-unmatch", "-z", "--", raw)
            tracked_names = [entry.decode("utf-8", "surrogateescape") for entry in tracked.stdout.split(b"\0") if entry]
            if tracked.returncode or tracked_names != [raw]:
                raise IntegrationGuardError(f"path must name an existing file or exact tracked deletion: {raw!r}")
        if raw in seen:
...
def _published_binary_full_index_sha256(repo: Path | str, commit: str) -> str:
    root = repository_root(repo)
    result = _git(root, "diff-tree", "--no-commit-id", "--binary", "--full-index", "--no-ext-diff", "-r", f"{commit}^", commit, "--")
    if result.returncode:
...
def run_transaction(
    ...
    """
    root = repository_root(repo)
    literal_paths = validate_literal_paths(root, paths)
    ...
    with held_transaction_lock(root, record):
        ...
        if foreign:
            raise IntegrationGuardError(f"foreign staged paths refused without mutation: {', '.join(sorted(foreign))}")
        added = _git(root, "--literal-pathspecs", "add", "--", *literal_paths)
        if added.returncode:
