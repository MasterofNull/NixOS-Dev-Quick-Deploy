...
if [[ "$QDRANT_ONLY" != "true" ]]; then
  ...
  run_or_echo "${PROJECT_ROOT}/scripts/data/backup-postgresql.sh" restore "$postgres_backup" "$target_db"
...
if [[ "$POSTGRES_ONLY" != "true" ]]; then
  ...
  run_or_echo "${PROJECT_ROOT}/scripts/data/backup-qdrant.sh" restore "$qdrant_backup" "$target_collection"
