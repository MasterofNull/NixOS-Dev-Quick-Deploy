...
#
# Usage:
#   scripts/aq-knowledge-import.sh "what is lib.mkForce"
#   scripts/aq-knowledge-import.sh "NixOS flake"   [--project nixos-docs]
#
# Workflow:
...
if [[ -z "$AIDB_KEY" ]]; then
  echo "ERROR: AIDB_API_KEY or AIDB_API_KEY_FILE not set" >&2
  exit 1
fi
...
CONTENT="$(gemini -p "$PROMPT" 2>/dev/null)" || {
  echo "ERROR: gemini CLI failed. Is it installed? export PATH=\"\$HOME/.npm-global/bin:\$PATH\"" >&2
  exit 1
}

if [[ -z "$CONTENT" ]]; then
  echo "ERROR: gemini returned empty content" >&2
  exit 1
fi
