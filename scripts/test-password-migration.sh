...
test_grafana_login() {
    ...
    if [[ "$grafana_running" == "true" ]]; then
        ...
        if curl -s -u "admin:$grafana_password" http://localhost:${grafana_port}/api/org 2>/dev/null | grep -q "id"; then
        ...
        if curl -s -u "admin:admin" http://localhost:${grafana_port}/api/org 2>/dev/null | grep -q "id"; then
            test_result 1 "OLD PASSWORD STILL WORKS - SECURITY ISSUE!"
            return 1
        else
            test_result 0 "Old default password correctly rejected"
