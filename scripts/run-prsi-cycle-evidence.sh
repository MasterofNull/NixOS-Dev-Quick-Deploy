...
for i in $(seq 1 "${COUNT}"); do
  ...
  sync_json="$(run_json_last python3 "${ROOT_DIR}/scripts/prsi-orchestrator.py" sync --since="${SINCE_WINDOW}")"
  list_json="$(run_json_last python3 "${ROOT_DIR}/scripts/prsi-orchestrator.py" list)"

  cycle_cmd=(python3 "${ROOT_DIR}/scripts/prsi-orchestrator.py" cycle --since="${SINCE_WINDOW}" --execute-limit="${EXECUTE_LIMIT}")
  ...
  jq -n \
    ...
    '{
      ...
      proposal: {
        ...
        files: ["scripts/prsi-orchestrator.py", "config/runtime-prsi-policy.json"],
        expected_kpi_delta: [
  ...
  checks=(
    "syntax_or_lint:bash ${ROOT_DIR}/scripts/check-prsi-cycle-contract.sh"
    "runtime_contract:bash ${ROOT_DIR}/scripts/validate-runtime-declarative.sh"
    "report_schema:bash ${ROOT_DIR}/scripts/check-aq-report-contract.sh"
    "security_checks:bash ${ROOT_DIR}/scripts/check-api-auth-hardening.sh"
    "focused_smoke_or_eval:bash ${ROOT_DIR}/scripts/check-prsi-bootstrap-integrity.sh"
    "critical_regression_scan:bash ${ROOT_DIR}/scripts/check-prsi-validation-matrix.sh"
  )
