...
# Verify /control/ai-coordinator/status surfaces accepted lesson references.
...
TMP_DIR="$(mktemp -d /tmp/status-lesson-refs-XXXXXX)"
trap 'rm -rf "${TMP_DIR}"' EXIT

curl -fsS \
  -H "X-API-Key: ${HYBRID_API_KEY}" \
  "${HYBRID_URL}/control/ai-coordinator/status" > "${TMP_DIR}/status.json"

jq -e '.active_lesson_refs | length >= 1' "${TMP_DIR}/status.json" >/dev/null
jq -e '.agent_lessons.active_lessons | length >= 1' "${TMP_DIR}/status.json" >/dev/null

printf 'PASS: ai-coordinator status surfaces active lesson refs\n'
