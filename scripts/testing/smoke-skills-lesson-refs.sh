...
# Verify /control/ai-coordinator/skills surfaces accepted lesson references.
...
TMP_DIR="$(mktemp -d /tmp/skills-lesson-refs-XXXXXX)"
trap 'rm -rf "${TMP_DIR}"' EXIT

curl -fsS \
  -H "X-API-Key: ${HYBRID_API_KEY}" \
  "${HYBRID_URL}/control/ai-coordinator/skills" > "${TMP_DIR}/skills.json"

jq -e '.active_lesson_refs | length >= 1' "${TMP_DIR}/skills.json" >/dev/null
jq -e '.shared_skill_registry.available == true' "${TMP_DIR}/skills.json" >/dev/null

printf 'PASS: ai-coordinator skills surface active lesson refs\n'
