...
def _check_c6a_authorize_launch_coverage(ctx: RunContext) -> list[CheckResult]:
    """0.10.53: Foundation C C6a -- authorize_launch/consume_launch single-use
    launch token on the C6-S launch socket: issue->consume->duplicate-deny->
    non-TEG-deny->epoch-superseded-deny integration exercise, dashboard-visible."""
    check = ctx.repo_root / "scripts" / "testing" / "test-c6a-authorize-launch-service-coverage.py"
    if not check.exists():
        return [failed(1, "0.10.53", "C6a authorize-launch coverage", "test-c6a-authorize-launch-service-coverage.py missing")]
    proc = subprocess.run(
        ["python3", str(check)],
        cwd=ctx.repo_root,
        text=True,
    ...
    if proc.returncode == 0:
        return [passed(1, "0.10.53", "C6a authorize-launch coverage: issue->consume->duplicate-deny->non-TEG-deny->epoch-superseded-deny")]
    detail = (proc.stdout + proc.stderr).strip() or f"exit {proc.returncode}"
...
def _check_factory_gate_retrofit(ctx: RunContext) -> list[CheckResult]:
    """0.10.46: existing-project preservation and confirmation fixtures."""
    description = "Factory retrofit fixture: preview, confirmation, originals and hook composition"
    checker = ctx.repo_root / "scripts/testing/test-factory-gate-retrofit.py"
    if not checker.is_file():
        return [failed(5, "0.10.46", description, "focused test missing")]
    try:
        probe = subprocess.run(
            ["python3", str(checker)], cwd=ctx.repo_root,
            capture_output=True, text=True, timeout=120,
        ...
        if probe.returncode:
            return [failed(5, "0.10.46", description, f"fixture exit {probe.returncode}")]
        prefix = "AQ_QA_FACTORY_RETROFIT_FIXTURE="
        markers = [line[len(prefix):] for line in probe.stdout.splitlines() if line.startswith(prefix)]
        if len(markers) != 1:
            raise ValueError("expected one fixture marker")
        evidence = json.loads(markers[0])
        invariants = (
            "preview_read_only", "confirmation_enforced",
            "originals_preserved", "hooks_composed",
        )
        if not isinstance(evidence, dict) or any(evidence.get(key) is not True for key in invariants):
            raise ValueError("fixture invariant evidence is incomplete")
    except (OSError, subprocess.TimeoutExpired, ValueError) as error:
