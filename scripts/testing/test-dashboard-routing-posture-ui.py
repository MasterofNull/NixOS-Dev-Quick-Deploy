...
def main() -> int:
    ...
    assert_true("/api/aistack/routing/decisions?limit=25" in html, "expected dashboard to expose recent routing decision feed endpoint")
    ...
    assert_true('@router.get("/routing/decisions")' in route, "expected routing decisions API route")
    assert_true('"recent_decisions": recent_decisions_enriched' in route or '"recent_decisions": recent_decisions_enriched,' in route, "expected routing summary to expose recent decisions")
    ...
    assert_true('"items": decisions' in route or '"items": decisions,' in route, "expected routing decisions API to expose filtered decision items")
