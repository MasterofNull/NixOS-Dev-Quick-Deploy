...
usage() {
  cat <<'EOF'
Usage: ./scripts/testing/check-package-count-drift.sh [options]
...
if [[ ! -f "$BASELINE_PATH" ]]; then
  ...
  echo "Run ./scripts/testing/check-package-count-drift.sh --write-baseline to create it." >&2
  exit 1
fi
...
if ! diff -u "$BASELINE_PATH" "$tmp_file" >/dev/null; then
  ...
  echo "  ./scripts/testing/check-package-count-drift.sh --write-baseline" >&2
  exit 1
fi
