...
def main() -> None:
    ...
    require('timer_state="$(_systemd_unit_state "$timer")"' in bash, "timer checks must use systemd show fallback")
    require('$(_systemd_unit_state apparmor.service)' in bash, "AppArmor check must use systemd show fallback")
    ...
    require("_http_post_json()" in bash, "bash QA must provide Python HTTP POST fallback for curl probe failures")
    ...
    require('_http_post_json "${HYBRID_URL}/v1/orchestrate"' in bash, "front-door smoke must use sandbox-aware POST helper")
    require('_http_post_json "${AIDB_URL}/vector/search"' in bash, "AIDB vector checks must use sandbox-aware POST helper")
    require('/api/aistack/agent-runs/swimlane' in bash and '_http_get "http://127.0.0.1:${dash_port}' in bash,
            "dashboard route checks must use sandbox-aware HTTP helper")
