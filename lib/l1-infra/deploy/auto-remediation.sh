...
run_health_checks_remediation() {
  ...
  # Run health check
  if [[ -f "${REPO_ROOT}/lib/l1-infra/deploy/parallel-health-checks.sh" ]]; then
    if bash -c "source ${REPO_ROOT}/lib/l1-infra/deploy/parallel-health-checks.sh; wait_for_service_ready '${service}' 'http://localhost:8080/health'" 2>&1 | tee -a "${REMEDIATION_CONFIG_DIR}/${remediation_id}.log"; then
