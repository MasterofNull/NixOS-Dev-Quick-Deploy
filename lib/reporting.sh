...
declare -a REPORTING_SKIP_DECLARATIVE_UNITS=(
...
_reporting_skip_declarative_check() {
    local unit="$1"
    local skip_unit
    local normalized="${unit%.service}"
    for skip_unit in "${REPORTING_SKIP_DECLARATIVE_UNITS[@]}"; do
...
inspect_single_service_unit() {
    ...
    if [[ "$has_nixos_option" == true && "$unit" == *.service && "$unit" != *@* ]]; then
        if _reporting_skip_declarative_check "$unit"; then
    ...
    if [[ "$has_systemd_analyze" == true ]]; then
        ...
        if [[ -n "$verify_output" ]]; then
            _report_systemd_verify_output "$unit" "$verify_output"
        else
