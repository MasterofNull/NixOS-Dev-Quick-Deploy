...
@test "ERR_AI_STACK_DEPLOY is 50" {
    [[ "$ERR_AI_STACK_DEPLOY" -eq 50 ]]
}
...
@test "all error codes are unique" {
    local codes=(
        ...
        "$ERR_AI_STACK_DEPLOY" "$ERR_AI_STACK_RUNTIME" "$ERR_AI_STACK_NAMESPACE"
        "$ERR_AI_STACK_MANIFEST" "$ERR_IMAGE_BUILD" "$ERR_IMAGE_IMPORT"
