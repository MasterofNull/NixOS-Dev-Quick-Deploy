...
## Issues Backlog
- Active backlog: `.agent/memory/issues-backlog.md` (latest: dogfood collision cleanup fixed; local agent-mode review can retain the switchboard slot, compact local planning needs source verification, and a Claude review dispatch orphaned empty; cancel-state and exact-diff-hash tooling gaps remain open)
