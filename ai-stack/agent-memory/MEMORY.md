...
## Issues Backlog
- Active backlog: `.agent/memory/issues-backlog.md` (latest: local inference adversarial hardening—replay fidelity in flight; llama confinement and fail-closed lane health pending rebuild; promotion, Playwright truth, L2B drift, and audit entrypoint repaired—plus review convergence, integration isolation, staged-index drift, subagent observability, and unsafe pre-fence attention approval)
