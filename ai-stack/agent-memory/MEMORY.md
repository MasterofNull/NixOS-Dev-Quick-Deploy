...
## Issues Backlog
- Active backlog: `.agent/memory/issues-backlog.md` (latest: review-revision convergence soft failure, coordinator substring-accept bypass, HERDR H2A incomplete-green oracles, canonical event sandbox recurrence, staged-index ownership/drift, subagent observability, and unsafe pre-fence attention approval; plus provider, isolation, recursive-feedback, and receipt enforcement)
