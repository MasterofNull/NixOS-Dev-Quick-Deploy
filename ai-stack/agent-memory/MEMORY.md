...
## Issues Backlog
- Active backlog: `.agent/memory/issues-backlog.md` (latest: Foundation C2 enforcement contract gaps, pre-commit unstaged-subject leakage, Codex effective-config hook regression, and collaboration-skill validator drift; also tracker/C0.3/C0.6 refactor gates, local deadline/slot leaks, provider no-progress, isolation, recursive feedback, and receipt enforcement)
