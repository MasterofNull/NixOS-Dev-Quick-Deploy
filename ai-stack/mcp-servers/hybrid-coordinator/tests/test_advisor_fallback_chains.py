...
class TestAdvisorFallbackChains:
    ...
    def test_consult_advisor_fallback_on_error(self):
    ...
    def test_consult_advisor_all_fallbacks_exhausted(self):
    ...
    def test_fallback_rank_in_metrics(self, tmp_path):
        """Test that fallback rank is included in advisor metrics."""
        metrics_db = str(tmp_path / "test_metrics.db")
        router = LLMRouter()
        
        # Use test db
        with patch.object(router, 'metrics_db', metrics_db):
            ...
            conn.execute("""
                CREATE TABLE IF NOT EXISTS advisor_consultations (
                    task_id TEXT, decision_type TEXT, executor_tier TEXT, 
                    executor_model TEXT, advisor_model TEXT, advisor_tokens INTEGER,
                    advisor_cost REAL, time_to_consult_ms INTEGER, question TEXT,
                    guidance_summary TEXT, task_success BOOLEAN, fallback_rank INTEGER
                )
            """)
            conn.execute("""
            ...
            # Ensure advisor is enabled in mock config
            with patch.object(router, 'advisor_enabled', True):
                # Get metrics
