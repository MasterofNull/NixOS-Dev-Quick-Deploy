...
class LLMRouterTests(unittest.TestCase):
    ...
    def test_execute_free_uses_coordinator_delegate(self):
        async def run_test():
            with patch.object(
                self.router,
    ...
    def test_execute_paid_uses_reasoning_profile(self):
        async def run_test():
            with patch.object(
                self.router,
                "_execute_via_coordinator",
    ...
    def test_execute_with_routing_escalates_from_local_to_free(self):
        async def run_test():
    ...
    def test_build_prompt_includes_advisor_guidance(self):
        ...
        self.assertIn("[Advisor guidance — modify]", prompt)
        self.assertIn("The prior mapping drifted", prompt)
