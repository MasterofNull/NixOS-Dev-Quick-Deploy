#!/usr/bin/env python3
"""
Circuit Breaker Implementation for Hybrid Coordinator

This module implements a circuit breaker pattern for service-to-service communication
within the Hybrid Coordinator to prevent cascading failures.
"""
...
# Example usage for Hybrid Coordinator internal calls
class HybridCoordinatorClient:
    """Client for Hybrid Coordinator to communicate with other services with circuit breaker protection."""
    
    def __init__(self, base_url: str):
        self.base_url = base_url
        # Circuit breaker with 3 failures in a row, 30s timeout, 15s reset (more sensitive for internal calls)
    ...
    @circuit_breaker()
    async def call_aidb(self, query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """Call AIDB with circuit breaker protection."""
        import aiohttp
...
async def example_usage():
    ...
    client = HybridCoordinatorClient(os.getenv("AIDB_URL", ""))
    
    try:
        result = await client.call_aidb("test query")
