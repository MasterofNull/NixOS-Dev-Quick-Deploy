...
def _llama_args_to_cli(model_id: str, local_path: str, llama_args: dict) -> str:
    """Render a model's llama_args dict into a llama-server CLI string.

    Bool True → bare flag, bool False → omitted, str/int → --flag value.
    Callers can pass this directly to llama-server or write it to an env file.
    """
    FLAG_MAP = {
        "ctx_size": "--ctx-size",
...
def _write_llama_args_file(model_id: str, local_path: str, llama_args: dict) -> Path:
    """Persist llama-server args to ~/.local/share/nixos-ai-stack/llama-active-args.env."""
    args_dir = Path.home() / ".local" / "share" / "nixos-ai-stack"
    args_dir.mkdir(parents=True, exist_ok=True)
    env_file = args_dir / "llama-active-args.env"
    cli_fragment = _llama_args_to_cli(model_id, local_path, llama_args)
    content = (
        f"# Auto-generated by model_lifecycle_manager — do not edit manually\n"
        f"# Active model: {model_id}\n"
        f"# Model path:   {local_path}\n"
        f"LLAMA_CPP_EXTRA_ARGS='{cli_fragment}'\n"
        f"LLAMA_CPP_MODEL={local_path}\n"
    )
...
class ModelLifecycleManager:
    ...
    async def promote_model(self, model_id: str) -> Dict:
        ...
        if health_ok:
            ...
            cli_cmd = None
            try:
                args_file = _write_llama_args_file(model_id, local_path, llama_args)
                cli_cmd = _llama_args_to_cli(model_id, local_path, llama_args)
            except Exception as _ae:
                logger.warning("model_lifecycle: could not write llama args file: %s", _ae)
            result = {
                "success": True,
                ...
                "llama_cli": cli_cmd,
                "llama_args_file": str(args_file) if args_file else None,
            }
            if symlink_warn:
