...
async def main():
    ...
    # --- PID lockfile guard ---
    lockfile_path = Path(os.getenv("CONTINUOUS_LEARNING_LOCKFILE", "/tmp/continuous-learning.pid"))
    if lockfile_path.exists():
        try:
            existing_pid = int(lockfile_path.read_text().strip())
            os.kill(existing_pid, 0)
            # Signal 0 succeeded — process is alive
            logger.info("continuous_learning_already_running", extra={"pid": existing_pid})
            return 1
        except ProcessLookupError:
            logger.info("stale_lockfile_removed", extra={"path": str(lockfile_path)})
