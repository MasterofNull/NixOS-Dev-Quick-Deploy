...
class LocalAgentExecutor:
    ...
    def _get_system_prompt(self, agent_type: AgentType, tools: List[Dict]) -> str:
        ...
        _workflow_contract = (
            ...
            "- Do NOT re-read a file already in your context. Use start_line/end_line for CONTEXT-ONLY reads.\n"
            "  EXCEPTION: if the next step is write_file, read the FULL file ONCE (no start_line/end_line),\n"
            "  then call write_file IMMEDIATELY — reading the same file twice in a row will abort the task.\n"
            "  write_file is FULL-FILE REPLACEMENT — writing only the fix region destroys every other line.\n"
            ...
            "STEP 3: read_file(<target-file>)  ← Read FULL file ONCE (no start_line/end_line).\n"
            "        After this one read, IMMEDIATELY call write_file — no other tool in between.\n"
            "        Do NOT re-read the file. write_file replaces the ENTIRE file:\n"
            "        include ALL original lines plus your change in the write_file content.\n"
