...
class LocalAgentExecutor:
    ...
    def _get_system_prompt(self, agent_type: AgentType, tools: List[Dict]) -> str:
        ...
        _workflow_contract = (
            ...
            "- git commit message format: type(scope): desc + Co-Authored-By: AQ <noreply@harness.local>\n"
            ...
            "        → pick the first non-hardware OPEN issue; note its line number N\n"
            "        read_file('.agent/memory/issues-backlog.md', start_line=N, end_line=N+15)\n"
            ...
            "STEP 4: announce in ONE sentence 'Fixing: [OPEN] <title> — <description>', then proceed\n"
            "STEP 5: read_file(<target>) before editing; write_file to implement the fix\n"
            "STEP 6: run_command('python3 -m py_compile <file>') for Python; 'bash -n <script>' for shell\n"
            "STEP 7: run_command('scripts/governance/tier0-validation-gate.sh --pre-commit')\n"
            "STEP 8: git_add(<files>); git_commit 'type(scope): desc\\n\\nCo-Authored-By: AQ <noreply@harness.local>'\n"
            "THEN REPORT: what changed, what passed, what to review.\n"
