...
# Telemetry writes are opt-in by default for privacy.
TELEMETRY_OPT_IN="${AI_TELEMETRY_ENABLED:-false}"

# Check if we can write to the main telemetry directory
if [[ "$TELEMETRY_OPT_IN" == "true" ]] && [[ -w "$TELEMETRY_DIR" ]]; then
    WRITE_DIR="$TELEMETRY_DIR"
else
    echo "ℹ️  Telemetry opt-in disabled or main directory not writable"
    echo "   Writing test data to $TEST_TELEMETRY_DIR instead"
...
for i in {1..5}; do
    metadata='{"query_hash":"sha256:test-query","results_count":3,"response_time_ms":45,"cache_hit":false}'
    create_event "aidb" "context_retrieval" "$metadata" >> "$AIDB_FILE"
done

# Vector search events
for i in {1..3}; do
    metadata='{"search_term_hash":"sha256:test-search","similarity_threshold":0.8,"results":5,"collection":"codebase-context"}'
...
cat <<EOF >> "$RALPH_FILE"
{"event":"task_completed","task_id":"$task_id","status":"completed","total_iterations":2,"task":{"task_id":"$task_id","prompt":"sha256:test-prompt","output":"sha256:test-output","iteration":2,"backend":"aider","context":{"source":"test-telemetry-script"}},"timestamp":"$timestamp"}
EOF
