...
def k8s_cluster_reachable() -> bool:
    if not command_exists("kubectl"):
        return False
    try:
        result = run_kubectl(["version", "--request-timeout=5s"])
        return result.returncode == 0
    except Exception:
        return False
...
def k8s_rollout_suggestion(namespace: str, deployment: str) -> str:
    return (
        f"kubectl rollout restart deploy/{deployment} -n {namespace} && "
        f"kubectl rollout status deploy/{deployment} -n {namespace}"
    )
...
def k8s_check_http_service(
    ...
    if not deploy:
        ...
        return ServiceCheck(
            ...
            details={"suggestion": f"kubectl apply -k ai-stack/kubernetes && kubectl get deploy/{deployment} -n {namespace}"},
        )
    ...
    if ready < desired:
        return ServiceCheck(
            ...
            details={
                "desired": desired,
                "ready": ready,
                "suggestion": k8s_rollout_suggestion(namespace, deployment),
            },
        )
    ...
    try:
        ...
        return ServiceCheck(
            ...
            details={"suggestion": k8s_rollout_suggestion(namespace, deployment)},
        )
...
def main():
    ...
    mode = args.mode
    if mode == "auto":
        # Prefer Kubernetes mode when API is reachable. Namespace may not exist
        # yet during bootstrap, and that should not force Podman fallback.
        mode = "k8s" if (k8s_namespace_available(namespace) or k8s_cluster_reachable()) else "podman"
