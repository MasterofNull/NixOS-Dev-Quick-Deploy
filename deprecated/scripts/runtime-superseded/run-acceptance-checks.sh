...
AI_STACK_MODE="${AI_STACK_MODE:-k8s}"
ACCEPTANCE_PROFILE="${ACCEPTANCE_PROFILE:-standard}"
...
export AI_STACK_MODE

# Strict K3s profile: enable core runtime checks by default for CI/gates.
if [[ "${ACCEPTANCE_PROFILE}" == "k3s-strict" ]]; then
  [[ "${RUN_NETPOL_TEST}" == "false" ]] && RUN_NETPOL_TEST="true"
  [[ "${RUN_VECTOR_DIM_TEST}" == "false" ]] && RUN_VECTOR_DIM_TEST="true"
  [[ "${RUN_FEEDBACK_TEST}" == "false" ]] && RUN_FEEDBACK_TEST="true"
  [[ "${RUN_RESTART_BUDGET_TEST}" == "false" ]] && RUN_RESTART_BUDGET_TEST="true"
fi

k8s_api_reachable() {
  command -v kubectl >/dev/null 2>&1 || return 1
  kubectl --request-timeout=10s cluster-info >/dev/null 2>&1
}
...
if [[ "${AI_STACK_MODE}" == "k8s" || "${ACCEPTANCE_PROFILE}" == "k3s-strict" ]]; then
  if ! k8s_api_reachable; then
    fail "Kubernetes API preflight"
    warn "kubectl cannot reach the cluster. Set KUBECONFIG correctly and verify: kubectl --request-timeout=10s cluster-info"
    echo ""
    echo "✗ Acceptance checks completed with ${failures} failure(s)"
    exit 1
  fi
fi
