...
check_flatpak_app() {
    ...
    local info_output=""
    local install_scope=""
    if info_output=$(flatpak info --system "$app_id" 2>/dev/null); then
        install_scope="system"
    elif info_output=$(flatpak info --user "$app_id" 2>/dev/null); then
        install_scope="user"
    fi
...
run_all_checks() {
    ...
    # Auto-detect: if libvirtd is not a configured systemd unit the virtualization
    # role is disabled in the flake (roles.virtualization.enable = false).
    # Missing tools are informational only, not failures in that case.
    local virtualization_required=false
    if systemctl cat libvirtd.service >/dev/null 2>&1; then
        virtualization_required=true
    fi
    if [[ "${VIRTUALIZATION_REQUIRED_OVERRIDE:-}" == "true" ]]; then
        virtualization_required=true
    elif [[ "${VIRTUALIZATION_REQUIRED_OVERRIDE:-}" == "false" ]]; then
    ...
    # Check NixOS systemd-based initrd
    # Detection: /proc/cmdline does NOT contain "systemd" with systemd initrd.
    # The reliable marker is the boot journal: when boot.initrd.systemd.enable = true
    # is active, systemd runs inside the initrd and logs initrd-specific units
    # (initrd-parse-etc.service, "Reached target Initrd") into the boot journal.
    print_check "Systemd-based initrd (fast boot)"
    if journalctl -b 0 --no-pager -q 2>/dev/null \
            | grep -qE "initrd-parse-etc|Reached target.*Initrd"; then
        print_success "Systemd-based initrd active"
        print_detail "boot.initrd.systemd confirmed — fast boot enabled"
    elif [ -d "/run/systemd" ]; then
        print_warning "Traditional initrd (reboot to activate systemd initrd)"
        print_detail "Already configured in base.nix — takes effect after next reboot"
    else
    ...
    # Container managers (k3s/podman/docker) are intentionally unsupported.
    # AI stack health checks target native NixOS systemd services only.
    local _nix_ai_backend="llamacpp"
    if [[ "$DETAILED" == "true" && -n "$_nix_ai_backend" ]]; then
        print_detail "Declarative AI backend: ${_nix_ai_backend}"
    fi
    # Override AI_BACKEND from Nix config when not explicitly set in environment.
    if [[ -z "${AI_BACKEND:-}" && -n "$_nix_ai_backend" ]]; then
        AI_BACKEND="$_nix_ai_backend"
    fi
