...
wait_for_ai_stack_readiness() {
    ...
    # llamacpp backend: services managed by systemd, not Kubernetes.
    # Readiness was already verified by provision_llm_models().
    local _declared_backend
    _declared_backend=$(nix eval --raw \
        "path:${SCRIPT_DIR}#nixosConfigurations.${FLAKE_TARGET:-nixos-ai-dev}.config.mySystem.aiStack.backend" \
        2>/dev/null || echo "llamacpp")
    if [[ "$_declared_backend" == "llamacpp" ]]; then
        return 0
    fi
    ...
    local strict_gate="${K3S_READINESS_GATE:-false}"

    local timeout_secs="${K3S_READINESS_TIMEOUT:-300}"
    local required_deploys_csv="${K3S_REQUIRED_DEPLOYS:-aidb,qdrant,llama-cpp,postgres,redis}"
    local -a required_deploys=()
    IFS=',' read -r -a required_deploys <<< "$required_deploys_csv"
    ...
    while ! kubectl --request-timeout=15s cluster-info >/dev/null 2>&1; do
        ...
        if (( attempts >= max_attempts )); then
            if [[ "$strict_gate" == "true" ]]; then
                print_error "Kubernetes API not reachable; strict readiness gate failed."
                print_info "Verify cluster access with: kubectl --request-timeout=30s cluster-info"
                return 1
            fi
    ...
    if ! kubectl --request-timeout=30s -n "$namespace" get deploy >/dev/null 2>&1; then
        if [[ "$strict_gate" == "true" ]]; then
            print_error "Namespace '${namespace}' has no deployments; strict readiness gate failed."
            return 1
        fi
        return 0
    fi
    ...
    while true; do
        ...
        local deploy desired ready
        for deploy in "${required_deploys[@]}"; do
            deploy="${deploy// /}"
            [[ -n "$deploy" ]] || continue
            if ! kubectl --request-timeout=30s -n "$namespace" get deploy "$deploy" >/dev/null 2>&1; then
                all_ready=false
                if [[ "$strict_gate" != "true" ]]; then
                    print_warning "${deploy} deployment not found (non-fatal; reconciliation may still be running)."
                fi
                continue
            fi
            desired="$(kubectl --request-timeout=30s -n "$namespace" get deploy "$deploy" -o jsonpath='{.spec.replicas}' 2>/dev/null || echo 0)"
            ready="$(kubectl --request-timeout=30s -n "$namespace" get deploy "$deploy" -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo 0)"
            desired="${desired:-0}"
            ready="${ready:-0}"
            if [[ "$ready" -lt "$desired" ]]; then
                all_ready=false
                if [[ "$strict_gate" != "true" ]]; then
                    print_warning "${deploy} deployment not ready yet (${ready}/${desired}) (non-fatal)."
                fi
            fi
        done

        if [[ "$all_ready" == "true" ]]; then
            print_success "AI stack readiness checks passed."
            return 0
        fi
        ...
        if (( elapsed >= timeout_secs )); then
            if [[ "$strict_gate" == "true" ]]; then
                print_error "AI stack strict readiness gate timed out after ${timeout_secs}s."
                print_info "Inspect deployments with: kubectl --request-timeout=30s get deploy,pods -n ${namespace}"
                return 1
            fi
            print_warning "AI stack readiness wait timed out after ${timeout_secs}s (non-fatal)."
            return 0
        fi
        sleep 5
    done
...
# provision_llm_models: Trigger the declarative systemd fetch services to
# download any missing LLM/embedding models and wait for completion.
# Called after nixos-rebuild switch so the new units are already active.
#
# Strategy:
#   1. Query model paths from the active NixOS config via nix eval.
#   2. Skip any model that is already on disk (idempotent).
#   3. For missing models, start the corresponding fetch service and tail
#      the journal so the user sees download progress.
#   4. Start inference services once all models are ready.
provision_llm_models() {
    local nixos_target="${1:-nixos-ai-dev}"
    local flake_ref="${2:-path:${SCRIPT_DIR}}"

    print_section "LLM Model Provisioning"

    if ! command -v systemctl >/dev/null 2>&1; then
        print_warning "systemctl not available — skipping model provisioning"
        return 0
    fi

    # Resolve model paths from the evaluated NixOS config.
    local chat_model="" embed_model=""
    chat_model=$(nix eval --raw \
        "${flake_ref}#nixosConfigurations.${nixos_target}.config.mySystem.aiStack.llamaCpp.model" \
        2>/dev/null || true)
    embed_model=$(nix eval --raw \
        "${flake_ref}#nixosConfigurations.${nixos_target}.config.mySystem.aiStack.embeddingServer.model" \
        2>/dev/null || true)
    local embed_enabled=""
    embed_enabled=$(nix eval --raw \
        "${flake_ref}#nixosConfigurations.${nixos_target}.config.mySystem.aiStack.embeddingServer.enable" \
        2>/dev/null || echo "false")

    # ── Chat model ────────────────────────────────────────────────────────────
    # Helper: restart a RemainAfterExit fetch service and wait for it to finish.
    # systemctl restart is synchronous for oneshot services — it blocks until
    # ExecStart completes. Using restart (not start) forces re-execution even
    # when the service is already active (RemainAfterExit from a prior run).
    _fetch_model_service() {
        local unit="$1" model_path="$2" label="$3"
        print_info "${label} missing — downloading via ${unit}"
        print_info "  Target: ${model_path}"
        print_info "  Follow progress: journalctl -u ${unit} -f"
        # Clear any failed state before restart.
        sudo systemctl reset-failed "${unit}" 2>/dev/null || true
        if sudo systemctl restart "${unit}" 2>/dev/null; then
            # Prefer direct file check; fall back to inference-service active check
            # because /var/lib/llama-cpp/models/ is 0750 llama:llama and unreadable
            # by the deploying user.
            local infer_svc="${unit//-model-fetch/}"
            if [[ -f "${model_path}" ]] \
                    || sudo test -f "${model_path}" 2>/dev/null \
                    || systemctl is-active --quiet "${infer_svc}" 2>/dev/null; then
                print_success "${label} ready (${infer_svc} active)"
                return 0
            else
                print_warning "${label} not found after fetch service completed"
                print_info "  Check journal: journalctl -u ${unit} -n 30 --no-pager"
                print_info "  Retry manually: sudo systemctl restart ${unit}"
                return 1
            fi
        else
            print_warning "Could not restart ${unit} — check: journalctl -u ${unit} --no-pager -n 20"
            return 1
        fi
    }

    if [[ -n "$chat_model" ]]; then
        # Prefer service-active check: model dir is 0750 llama:llama and unreadable
        # by the deploying user, so -f checks may spuriously fail on a live system.
        if systemctl is-active --quiet llama-cpp.service 2>/dev/null; then
            print_success "Chat model loaded (llama-cpp.service active)"
        elif [[ -f "$chat_model" ]] || sudo test -f "$chat_model" 2>/dev/null; then
            print_success "Chat model already present: $chat_model"
        else
            _fetch_model_service "llama-cpp-model-fetch.service" "$chat_model" "Chat model" || true
        fi
    else
        print_info "Could not resolve chat model path from flake — skipping chat model provisioning"
    fi

    # ── Embedding model ───────────────────────────────────────────────────────
    if [[ "$embed_enabled" == "true" && -n "$embed_model" ]]; then
        if systemctl is-active --quiet llama-cpp-embed.service 2>/dev/null; then
            print_success "Embedding model loaded (llama-cpp-embed.service active)"
        elif [[ -f "$embed_model" ]] || sudo test -f "$embed_model" 2>/dev/null; then
            print_success "Embedding model already present: $embed_model"
        else
            _fetch_model_service "llama-cpp-embed-model-fetch.service" "$embed_model" "Embedding model" || true
        fi
    fi

    # ── Start inference services ───────────────────────────────────────────────
    # Models are on disk — ensure services are active. They may already be
    # running if this is a re-deploy; systemctl start is idempotent.
    print_info "Starting inference services..."
    sudo systemctl start llama-cpp.service 2>/dev/null || true
    if [[ "$embed_enabled" == "true" ]]; then
        sudo systemctl start llama-cpp-embed.service 2>/dev/null || true
    fi

    # Brief settle wait then status check.
    sleep 3
    local chat_active embed_active
    chat_active=$(systemctl is-active llama-cpp.service 2>/dev/null || true)
    if [[ "$chat_active" == "active" || "$chat_active" == "activating" ]]; then
        print_success "llama-cpp.service running (chat inference on :8080)"
    else
        print_warning "llama-cpp.service not yet active (state: ${chat_active}) — check: journalctl -u llama-cpp"
    fi

    if [[ "$embed_enabled" == "true" ]]; then
        embed_active=$(systemctl is-active llama-cpp-embed.service 2>/dev/null || true)
        if [[ "$embed_active" == "active" || "$embed_active" == "activating" ]]; then
            print_success "llama-cpp-embed.service running (embeddings on :8081)"
        else
            print_warning "llama-cpp-embed.service not yet active (state: ${embed_active})"
        fi
    fi
...
# ensure_mlops_credentials: Create /etc/ai-stack/mlops-credentials.env if it
# does not already exist. Prompts the user once for the MinIO admin password;
# subsequent runs skip entirely (idempotent). Must be called BEFORE rebuild so
# the Podman containers can read the file on first start.
ensure_mlops_credentials() {
    local nixos_target="${1:-nixos-ai-dev}"
    local flake_ref="${2:-path:${SCRIPT_DIR}}"
    local creds_file="/etc/ai-stack/mlops-credentials.env"

    # Only run when mlops is enabled in the target config.
    local mlops_enabled=""
    mlops_enabled=$(nix eval --raw \
        "${flake_ref}#nixosConfigurations.${nixos_target}.config.mySystem.mlops.enable" \
        2>/dev/null || echo "false")
    [[ "${mlops_enabled}" == "true" ]] || return 0

    # Idempotent — skip if already present.
    if [[ -f "${creds_file}" ]]; then
        print_info "MLops credentials already present: ${creds_file}"
        return 0
    fi

    print_section "MLOps Credentials Setup"
    print_info "MinIO and MLflow need a credentials file at: ${creds_file}"
    print_info "This is created once; subsequent deploys skip this step."

    local minio_user minio_password interactive=true
    [[ -t 0 ]] || interactive=false

    minio_user="${MINIO_ROOT_USER:-aistack-admin}"

    if [[ "${interactive}" == "true" ]]; then
        while true; do
            minio_password=$(prompt_secret "MinIO admin password (12+ chars)")
            if [[ ${#minio_password} -ge 12 ]]; then break; fi
            print_warning "Password must be at least 12 characters."
        done
    else
        # Non-interactive: require env var.
        if [[ -z "${MINIO_ROOT_PASSWORD:-}" ]]; then
            print_error "Non-interactive session: set MINIO_ROOT_PASSWORD env var and rerun."
            return 1
        fi
        minio_password="${MINIO_ROOT_PASSWORD}"
    fi

    print_info "Writing ${creds_file} (sudo required)..."
    ...
    print_success "Credentials written to ${creds_file}"
    return 0
}

# provision_mlops: Wait for MinIO and MLflow containers to be healthy after
# nixos-rebuild switch, ensure bucket init ran, and configure the DVC remote.
# Called post-rebuild; credentials file is guaranteed to exist by this point.
provision_mlops() {
    local nixos_target="${1:-nixos-ai-dev}"
    local flake_ref="${2:-path:${SCRIPT_DIR}}"
    ...
    print_section "MLOps Stack Provisioning"
    ...
    for i in $(seq 1 30); do
        if curl -sf "http://127.0.0.1:9000/minio/health/live" >/dev/null 2>&1; then
            print_success "MinIO healthy (API :9000, console :9001)"
            break
        fi
        sleep 2
        if [[ $i -eq 30 ]]; then
            print_warning "MinIO not ready after 60s — check: systemctl status podman-minio"
            return 0
        fi
    done

    # ── Bucket init ───────────────────────────────────────────────────────────
    if systemctl is-active --quiet mlops-minio-init.service 2>/dev/null; then
        print_success "MinIO buckets ready (mlops-minio-init already ran)"
    else
        print_info "Triggering bucket init..."
        sudo systemctl start mlops-minio-init.service 2>/dev/null || true
    fi

    # ── Wait for MLflow ───────────────────────────────────────────────────────
    print_info "Waiting for MLflow to be healthy..."
    for i in $(seq 1 30); do
        if curl -sf "http://127.0.0.1:5000/health" >/dev/null 2>&1; then
            print_success "MLflow healthy (UI :5000)"
            break
        fi
        sleep 2
        if [[ $i -eq 30 ]]; then
            print_warning "MLflow not ready after 60s — check: systemctl status podman-mlflow"
        fi
    done

    # ── DVC remote ───────────────────────────────────────────────────────────
    if command -v dvc >/dev/null 2>&1; then
        print_info "Configuring DVC remote → s3://dvc @ MinIO :9000"
        if "${SCRIPT_DIR}/scripts/setup-dvc-remote.sh" \
                --repo-dir "${SCRIPT_DIR}" \
                --minio-url "http://localhost:9000" 2>&1 | \
                grep -E "configured|ERROR|WARN" | head -5; then
            print_success "DVC remote configured (run 'dvc push' to verify)"
        fi
    fi
...
run_flake_first_deployment() {
    ...
    if [[ "$_ai_backend" == "k3s" ]]; then
        ...
        # llamacpp backend: ensure MLops credentials file exists before rebuild
        # so Podman containers can read it on first start.
        if ! ensure_mlops_credentials "$detected_host-ai-dev" "path:${SCRIPT_DIR}"; then
            print_error "Failed to prepare MLops credentials"
            return 1
        fi
    fi
    ...
    # Provision LLM models after a live switch — downloads any missing chat or
    # embedding model via the declarative systemd fetch services, then starts
    # the inference services. Skipped on boot/build (services not yet active).
    if [[ "$deploy_mode" == "switch" && "$_ai_backend" == "llamacpp" ]]; then
        if ! provision_llm_models "$nixos_target" "$flake_ref"; then
            print_warning "Model provisioning encountered an issue — AI stack may need manual start"
        fi
        if ! provision_mlops "$nixos_target" "$flake_ref"; then
            print_warning "MLops provisioning encountered an issue — check podman-minio / podman-mlflow"
        fi
    fi
...
run_flake_first_legacy_outcome_tasks() {
    ...
    if [[ "${RUN_FLAKE_FIRST_LEGACY_PARITY_TASKS}" != "true" ]]; then
        ...
        # Phase 9 imperative stack/model scripts skipped in flake-first mode.
        # Model provisioning is handled by provision_llm_models() called directly
        # from run_flake_first_deployment after deploy-clean in switch mode.
        return 0
    fi
...
run_post_deployment() {
    ...
    if [[ "$RUN_AI_MODEL" == true ]]; then
        local _post_backend
        _post_backend=$(nix eval --raw \
            "path:${SCRIPT_DIR}#nixosConfigurations.${FLAKE_TARGET:-nixos-ai-dev}.config.mySystem.aiStack.backend" \
            2>/dev/null || echo "llamacpp")
        print_section "Post-Deploy: Start Declarative AI Services"
        log INFO "Starting ai-stack.target via systemd"
        if sudo systemctl start ai-stack.target 2>/dev/null; then
            print_success "ai-stack.target start requested"
            if ! wait_for_ai_stack_readiness; then
                stack_exit=1
            fi
            echo ""
        else
            stack_exit=1
            print_warning "Failed to start ai-stack.target. Review: journalctl -u ai-stack.target -n 50 --no-pager"
            echo ""
        fi
    else
    ...
    if [[ $final_exit -eq 0 ]]; then
        ...
        if [[ -x "$health_monitor" && "${SKIP_AI_MODEL:-false}" != true ]]; then
            ...
            if [[ "${_post_backend:-llamacpp}" == "k3s" ]]; then
                print_info "To check stack status, run: kubectl get pods -n ai-stack"
            else
                print_info "To check AI stack: systemctl status llama-cpp.service open-webui.service"
                print_info "Or run the health check: $health_monitor"
            fi
            echo ""
        fi
    else
        ...
        if [[ $stack_exit -ne 0 ]]; then
            print_info "AI stack startup failed. Run: sudo systemctl status ai-stack.target --no-pager"
        fi
    fi

    # ── Guardrail alert backlog ────────────────────────────────────────────────
    # Show any pending alerts from the current deploy cycle, then clear them.
    # A successful rebuild implies the system is now in a good state; stale
    # alerts from prior cycles (e.g. transient timer races) are not actionable.
    local _alert_dir="/var/lib/nixos-quick-deploy/alerts"
    if [[ -d "$_alert_dir" ]]; then
        local _alert_files=()
        mapfile -t _alert_files < <(find "$_alert_dir" -maxdepth 1 -type f 2>/dev/null | sort)
        if [[ ${#_alert_files[@]} -gt 0 ]]; then
            print_warning "${#_alert_files[@]} guardrail alert(s) from prior cycles:"
            for _f in "${_alert_files[@]}"; do
                print_info "  $(basename "$_f")"
                head -5 "$_f" 2>/dev/null | sed 's/^/    /' || true
            done
            print_info "Clearing stale alerts (deploy succeeded — system is converged)."
            sudo rm -f "${_alert_files[@]}" 2>/dev/null || \
                print_warning "Could not clear alerts (no sudo); clear manually: sudo rm -f ${_alert_dir}/*"
        fi
    fi
...
main() {
    ...
    if [[ "$FLAKE_FIRST_MODE" == true ]]; then
        if run_flake_first_deployment; then
            ...
            # Guard with || so set -e does not abort here; exit code is forwarded.
            local _rc=0
            run_post_deployment || _rc=$?
            exit $_rc
        fi
        exit 1
    fi
