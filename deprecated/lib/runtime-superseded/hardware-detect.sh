...
#   $7 = ai_stack_enabled ("true" | "false", optional, default false)
#   $8 = ai_backend ("ollama" | "k3s", optional, default "ollama")
#   $9 = ai_models_csv  (comma-separated ollama tags, optional,
#                        default "qwen2.5-coder:7b")
#   $10 = ai_ui_enabled ("true" | "false", optional, default "true")
#   $11 = ai_vector_db_enabled ("true" | "false", optional, default "false")
# ---------------------------------------------------------------------------
detect_and_write_hardware_facts() {
    ...
    local ai_stack_enabled="${7:-false}"
    local ai_backend="${8:-ollama}"
    local ai_models_csv="${9:-qwen2.5-coder:7b}"
    local ai_ui_enabled="${10:-true}"
    local ai_vector_db_enabled="${11:-false}"
    ...
    # Convert comma-separated model list into a Nix list literal.
    # Input:  "qwen2.5-coder:7b,phi3:mini"
    # Output: [ "qwen2.5-coder:7b" "phi3:mini" ]
    local ai_models_nix='[ "qwen2.5-coder:7b" ]'
    if [[ -n "$ai_models_csv" ]]; then
        local _models_nix_inner=""
        IFS=',' read -ra _model_arr <<< "$ai_models_csv"
        for _m in "${_model_arr[@]}"; do
            _m="${_m#"${_m%%[![:space:]]*}"}"  # ltrim whitespace
            _m="${_m%"${_m##*[![:space:]]}"}"  # rtrim whitespace
            [[ -z "$_m" ]] && continue
            _models_nix_inner+=" \"${_m}\""
        done
        ai_models_nix="[${_models_nix_inner} ]"
    fi

    # Derive roles.aiStack.enable from ai_stack_enabled (keep in sync).
    local roles_ai_stack_nix="${ai_stack_enabled}"

    # Only set aiStack role to true when the ai-dev profile is NOT selected,
    # because ai-dev.nix already enables it via lib.mkDefault.  For other
    # profiles this provides the authoritative setting.
    if [[ "$selected_profile" == "ai-dev" ]]; then
        roles_ai_stack_nix="true"   # ai-dev always enables aiStack
    fi
...
{ ... }:
{
  mySystem = {
    ...
    roles.aiStack.enable = ${roles_ai_stack_nix};

    # AI stack configuration — only meaningful when roles.aiStack.enable = true.
    aiStack = {
      backend             = "${ai_backend}";
      acceleration        = "auto";
...
EOF
    ...
    printf "  AI stack:\n"
    printf "    Enabled:       %s\n" "$ai_stack_enabled"
    if [[ "$ai_stack_enabled" == "true" ]]; then
        printf "    Backend:       %s\n" "$ai_backend"
        printf "    Models:        %s\n" "$ai_models_csv"
        printf "    Web UI:        %s\n" "$ai_ui_enabled"
        printf "    Vector DB:     %s\n" "$ai_vector_db_enabled"
    fi
