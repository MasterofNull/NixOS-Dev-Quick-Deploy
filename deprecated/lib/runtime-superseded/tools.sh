...
#
# Exports:
#   - install_claude_code_native() → Install Claude Code CLI (native)
#   - install_gemini_cli_native() → Install Gemini CLI (native)
#   - install_qwen_code_native() → Install Qwen Code CLI (native)
#   - install_codex_cli_native() → Install Codex CLI (native)
#   - install_claude_code() → Install all AI Coding CLIs
...
# ============================================================================
# Install Gemini CLI via Native Installer
# ============================================================================
# Google Gemini CLI native installer for AI-assisted coding.
#
# The native installer:
#   - Downloads the correct binary for the current OS/arch
#   - Installs to ~/.local/bin/gemini
#   - Supports auto-updates
#
# Reference: https://github.com/google-gemini/gemini-cli
# ============================================================================
install_gemini_cli_native() {
    local gemini_bin="${PRIMARY_HOME:-$HOME}/.local/bin/gemini"
    local gemini_alt="${PRIMARY_HOME:-$HOME}/.gemini/bin/gemini"

    # Check if already installed
    if [ -x "$gemini_bin" ] && [ "$FORCE_UPDATE" != true ]; then
        local current_version
        current_version=$("$gemini_bin" --version 2>/dev/null || echo "unknown")
        if [[ "$current_version" != "unknown" ]]; then
            print_success "Gemini CLI already installed (${current_version})"
            return 0
        fi
    fi
    ...
    local install_exit=0
    local tmp_root="${TMPDIR:-/${TMP_FALLBACK:-tmp}}"
    local log_file="${tmp_root}/gemini-native-install.log"

    print_info "Installing Gemini CLI via native installer..."

    while (( attempt <= max_attempts )); do
        local installer_tmp
        installer_tmp="$(mktemp -p "$tmp_root" gemini-install.XXXXXX.sh)"
        local installer_hash=""
        local expected_hash="${GEMINI_INSTALLER_SHA256:-}"

        if command -v curl_safe >/dev/null 2>&1; then
            if ! curl_safe -fsSL "https://gemini.google.com/install.sh" -o "$installer_tmp"; then
                print_warning "Failed to download Gemini installer"
                rm -f "$installer_tmp"
                install_exit=1
                break
            fi
        else
            if ! curl --max-time 120 --connect-timeout 10 -fsSL "https://gemini.google.com/install.sh" -o "$installer_tmp"; then
                print_warning "Failed to download Gemini installer"
                rm -f "$installer_tmp"
                install_exit=1
                break
        ...
        if [[ -n "$installer_hash" ]]; then
            print_info "Gemini installer SHA-256: $installer_hash"
        fi

        if [[ -n "$expected_hash" && -n "$installer_hash" && "$installer_hash" != "$expected_hash" ]]; then
            print_error "Gemini installer hash mismatch (expected $expected_hash)"
            rm -f "$installer_tmp"
            install_exit=1
            break
        fi
        ...
        if [[ "${NONINTERACTIVE:-false}" != "true" ]] && declare -F confirm >/dev/null 2>&1; then
            if ! confirm "Run Gemini installer script from https://gemini.google.com/install.sh?" "n"; then
                print_warning "Gemini installer aborted by user"
                rm -f "$installer_tmp"
                install_exit=1
                break
            fi
        ...
        # Download installer to temp file first to verify integrity before execution
        local installer_url="https://gemini.google.com/install.sh"
        local download_tmp="${TMPDIR:-/tmp}/gemini-installer-$$.sh"

        if ! curl --max-time 30 --connect-timeout 10 -fsSL "$installer_url" -o "$download_tmp"; then
            print_error "Failed to download Gemini installer from $installer_url"
            install_exit=1
            rm -f "$download_tmp" 2>/dev/null || true
            break
        fi
        ...
        if (( attempt < max_attempts )); then
            print_warning "Gemini CLI install attempt $attempt/$max_attempts failed; retrying in ${timeout}s"
            sleep "$timeout"
            timeout=$(( timeout * ${RETRY_BACKOFF_MULTIPLIER:-2} ))
        fi
    ...
    if (( install_exit != 0 )); then
        print_warning "Gemini CLI native installation failed after $max_attempts attempts (exit code $install_exit)"
        print_detail "See $log_file for details"
        print_detail "Manual install: curl -fsSL --max-time 30 --connect-timeout 5 https://gemini.google.com/install.sh | bash"
        return 1
    fi
    ...
    if [ -x "$gemini_bin" ]; then
        actual_bin="$gemini_bin"
    elif [ -x "$gemini_alt" ]; then
        actual_bin="$gemini_alt"
        ln -sf "$gemini_alt" "$gemini_bin" 2>/dev/null || true
    fi

    if [ -z "$actual_bin" ]; then
        print_warning "Gemini CLI installer completed but binary not found at expected paths"
        print_detail "Checked: $gemini_bin, $gemini_alt"
        print_detail "Try: which gemini"
        return 1
    fi

    local installed_version
    installed_version=$("$actual_bin" --version 2>/dev/null || echo "unknown")
    print_success "Gemini CLI installed (${installed_version}) at $actual_bin"

    return 0
}

# ============================================================================
# Install Qwen Code CLI via Native Installer
# ============================================================================
# Alibaba Qwen Code CLI native installer for AI-assisted coding.
#
# The native installer:
#   - Downloads the correct binary for the current OS/arch
#   - Installs to ~/.local/bin/qwen
#   - Supports auto-updates
#
# Reference: https://github.com/QwenLM/qwen-code
# ============================================================================
install_qwen_code_native() {
    local qwen_bin="${PRIMARY_HOME:-$HOME}/.local/bin/qwen"
    local qwen_alt="${PRIMARY_HOME:-$HOME}/.qwen-code/bin/qwen"

    # Check if already installed
    if [ -x "$qwen_bin" ] && [ "$FORCE_UPDATE" != true ]; then
        local current_version
        current_version=$("$qwen_bin" --version 2>/dev/null || echo "unknown")
        if [[ "$current_version" != "unknown" ]]; then
            print_success "Qwen Code already installed (${current_version})"
            return 0
        fi
    fi
    ...
    local install_exit=0
    local tmp_root="${TMPDIR:-/${TMP_FALLBACK:-tmp}}"
    local log_file="${tmp_root}/qwen-native-install.log"

    print_info "Installing Qwen Code via native installer..."

    while (( attempt <= max_attempts )); do
        local installer_tmp
        installer_tmp="$(mktemp -p "$tmp_root" qwen-install.XXXXXX.sh)"
        local installer_hash=""
        local expected_hash="${QWEN_INSTALLER_SHA256:-}"

        if command -v curl_safe >/dev/null 2>&1; then
            if ! curl_safe -fsSL "https://qwen.ai/install.sh" -o "$installer_tmp"; then
                print_warning "Failed to download Qwen installer"
                rm -f "$installer_tmp"
                install_exit=1
                break
            fi
        else
            if ! curl --max-time 120 --connect-timeout 10 -fsSL "https://qwen.ai/install.sh" -o "$installer_tmp"; then
                print_warning "Failed to download Qwen installer"
                rm -f "$installer_tmp"
                install_exit=1
                break
        ...
        if [[ -n "$installer_hash" ]]; then
            print_info "Qwen installer SHA-256: $installer_hash"
        fi

        if [[ -n "$expected_hash" && -n "$installer_hash" && "$installer_hash" != "$expected_hash" ]]; then
            print_error "Qwen installer hash mismatch (expected $expected_hash)"
            rm -f "$installer_tmp"
            install_exit=1
            break
        fi
        ...
        if [[ "${NONINTERACTIVE:-false}" != "true" ]] && declare -F confirm >/dev/null 2>&1; then
            if ! confirm "Run Qwen Code installer script from https://qwen.ai/install.sh?" "n"; then
                print_warning "Qwen installer aborted by user"
                rm -f "$installer_tmp"
                install_exit=1
                break
            fi
        ...
        # Download installer to temp file first to verify integrity before execution
        local installer_url="https://qwen.ai/install.sh"
        local download_tmp="${TMPDIR:-/tmp}/qwen-installer-$$.sh"

        if ! curl --max-time 30 --connect-timeout 10 -fsSL "$installer_url" -o "$download_tmp"; then
            print_error "Failed to download Qwen installer from $installer_url"
            install_exit=1
            rm -f "$download_tmp" 2>/dev/null || true
            break
        fi
        ...
        if (( attempt < max_attempts )); then
            print_warning "Qwen Code install attempt $attempt/$max_attempts failed; retrying in ${timeout}s"
            sleep "$timeout"
            timeout=$(( timeout * ${RETRY_BACKOFF_MULTIPLIER:-2} ))
        fi
    ...
    if (( install_exit != 0 )); then
        print_warning "Qwen Code native installation failed after $max_attempts attempts (exit code $install_exit)"
        print_detail "See $log_file for details"
        print_detail "Manual install: curl -fsSL --max-time 30 --connect-timeout 5 https://qwen.ai/install.sh | bash"
        return 1
    fi
    ...
    if [ -x "$qwen_bin" ]; then
        actual_bin="$qwen_bin"
    elif [ -x "$qwen_alt" ]; then
        actual_bin="$qwen_alt"
        ln -sf "$qwen_alt" "$qwen_bin" 2>/dev/null || true
    fi

    if [ -z "$actual_bin" ]; then
        print_warning "Qwen Code installer completed but binary not found at expected paths"
        print_detail "Checked: $qwen_bin, $qwen_alt"
        print_detail "Try: which qwen"
        return 1
    fi

    local installed_version
    installed_version=$("$actual_bin" --version 2>/dev/null || echo "unknown")
    print_success "Qwen Code installed (${installed_version}) at $actual_bin"

    return 0
}

# ============================================================================
# Install OpenAI Codex CLI via Native Installer
# ============================================================================
# OpenAI Codex CLI native installer for AI-assisted coding.
#
# The native installer:
#   - Downloads the correct binary for the current OS/arch
...
install_codex_cli_native() {
    local codex_bin="${PRIMARY_HOME:-$HOME}/.local/bin/codex"
    local codex_alt="${PRIMARY_HOME:-$HOME}/.codex/bin/codex"

    # Check if already installed
    if [ -x "$codex_bin" ] && [ "$FORCE_UPDATE" != true ]; then
        local current_version
        current_version=$("$codex_bin" --version 2>/dev/null || echo "unknown")
        if [[ "$current_version" != "unknown" ]]; then
            print_success "Codex CLI already installed (${current_version})"
            return 0
        fi
    fi
    ...
    local install_exit=0
    local tmp_root="${TMPDIR:-/${TMP_FALLBACK:-tmp}}"
    local log_file="${tmp_root}/codex-native-install.log"

    print_info "Installing Codex CLI via native installer..."

    while (( attempt <= max_attempts )); do
        local installer_tmp
        installer_tmp="$(mktemp -p "$tmp_root" codex-install.XXXXXX.sh)"
        local installer_hash=""
        local expected_hash="${CODEX_INSTALLER_SHA256:-}"

        if command -v curl_safe >/dev/null 2>&1; then
            if ! curl_safe -fsSL "https://codex.openai.com/install.sh" -o "$installer_tmp"; then
                print_warning "Failed to download Codex installer"
                rm -f "$installer_tmp"
                install_exit=1
                break
            fi
        else
            if ! curl --max-time 120 --connect-timeout 10 -fsSL "https://codex.openai.com/install.sh" -o "$installer_tmp"; then
                print_warning "Failed to download Codex installer"
                rm -f "$installer_tmp"
                install_exit=1
                break
        ...
        if [[ -n "$installer_hash" ]]; then
            print_info "Codex installer SHA-256: $installer_hash"
        fi

        if [[ -n "$expected_hash" && -n "$installer_hash" && "$installer_hash" != "$expected_hash" ]]; then
            print_error "Codex installer hash mismatch (expected $expected_hash)"
            rm -f "$installer_tmp"
            install_exit=1
            break
        fi
        ...
        if [[ "${NONINTERACTIVE:-false}" != "true" ]] && declare -F confirm >/dev/null 2>&1; then
            if ! confirm "Run Codex CLI installer script from https://codex.openai.com/install.sh?" "n"; then
                print_warning "Codex installer aborted by user"
                rm -f "$installer_tmp"
                install_exit=1
                break
            fi
        ...
        # Download installer to temp file first to verify integrity before execution
        local installer_url="https://codex.openai.com/install.sh"
        local download_tmp="${TMPDIR:-/tmp}/codex-installer-$$.sh"

        if ! curl --max-time 30 --connect-timeout 10 -fsSL "$installer_url" -o "$download_tmp"; then
            print_error "Failed to download Codex installer from $installer_url"
            install_exit=1
            rm -f "$download_tmp" 2>/dev/null || true
            break
        fi
        ...
        if (( attempt < max_attempts )); then
            print_warning "Codex CLI install attempt $attempt/$max_attempts failed; retrying in ${timeout}s"
            sleep "$timeout"
            timeout=$(( timeout * ${RETRY_BACKOFF_MULTIPLIER:-2} ))
        fi
    ...
    if (( install_exit != 0 )); then
        print_warning "Codex CLI native installation failed after $max_attempts attempts (exit code $install_exit)"
        print_detail "See $log_file for details"
        print_detail "Manual install: curl -fsSL --max-time 30 --connect-timeout 5 https://codex.openai.com/install.sh | bash"
        return 1
    fi
    ...
    local actual_bin=""
    if [ -x "$codex_bin" ]; then
        actual_bin="$codex_bin"
    elif [ -x "$codex_alt" ]; then
        actual_bin="$codex_alt"
        ln -sf "$codex_alt" "$codex_bin" 2>/dev/null || true
    fi

    if [ -z "$actual_bin" ]; then
        print_warning "Codex CLI installer completed but binary not found at expected paths"
        print_detail "Checked: $codex_bin, $codex_alt"
        print_detail "Try: which codex"
        return 1
    fi

    local installed_version
    installed_version=$("$actual_bin" --version 2>/dev/null || echo "unknown")
    print_success "Codex CLI installed (${installed_version}) at $actual_bin"
...
download_llama_cpp_models_if_needed() {
    ...
    print_info "Skipping imperative model download helper (deprecated)."
    print_info "Models are provisioned declaratively by llama-cpp-model-fetch.service and llama-cpp-embed-model-fetch.service."
    return 0
}
