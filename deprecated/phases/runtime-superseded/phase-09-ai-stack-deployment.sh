...
phase_09_ai_stack_deployment() {
    ...
    if [ ! -f "${AI_STACK_DATA}/llama-cpp-models/${model_file}" ]; then
        ...
        log_info "Model artifacts are declarative and hash-pinned via Nix; no runtime downloader."
    else
...
🔧 Helper Scripts:
  • Health check:  bash /scripts/ai-stack-health.sh
  • Stack manage:  bash ${SCRIPT_DIR}/scripts/ai-stack-manage.sh [start|stop|restart|status]
  • Dashboard:     systemctl status command-center-dashboard.service
