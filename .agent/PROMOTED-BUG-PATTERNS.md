...
### Agent sessions can invent non-existent switchboard profile names (CRITICAL)
Remote delegation agents may output profile names like `antigravity-collective` that don't exist in the switchboard. The switchboard returns HTTP 400 "not a valid model ID" for unknown profiles. Valid remote profiles: `remote-free` (Llama 3.3 70B free), `remote-gemini` (Gemini 2.5 Flash Lite — paid Gemini/antigravity lane), `remote-coding` (Claude Sonnet 4.5), `remote-reasoning` (Claude Sonnet 4.5), `remote-tool-calling` (o4-mini), `remote-default`. Antigravity agent (`delegate-to-antigravity`, `aq-antigravity-agent`) MUST use `remote-gemini` — NOT `remote-free` (that's the Llama free tier). Always verify against switchboard GET /health → profiles keys. Affected files: `scripts/ai/delegate-to-antigravity` `_PROFILE_MAP`, `scripts/ai/aq-antigravity-agent` `_MODE_PROFILE`, `ai-stack/local-agents/agent_executor.py` `_select_remote_profile()`.
...
### root coordinator shim files don't re-export private names via `import *` (CRITICAL)
Root shim files (`ai_coordinator.py`, `ai_coordinator_handlers.py`, etc. at the top of `hybrid-coordinator/`) all do `from extensions.<name> import *`. Python's `import *` silently skips names starting with `_`. Any consumer that does `from ai_coordinator import _PRIVATE_NAME` gets ImportError at runtime — **not at py_compile time** (py_compile only checks syntax). Fix: add the private name to the explicit re-import line in the root shim: `from extensions.ai_coordinator import ..., _PRIVATE_NAME`. Pattern: whenever adding a new private constant to `extensions/ai_coordinator.py` that needs to be importable by other root-level files, add it to the explicit list in the root shim in the same PR.
...
### `install -d -m MODE` on existing directories changes mode — do not use on managed paths (CRITICAL NixOS)
`install -d -m 0700 /path` for an EXISTING directory silently changes its mode (GNU coreutils behavior). This bit us: `cpp-dev.nix` ran `install -d -m 700 "$(dirname "$CLAUDE_JSON")"` where `CLAUDE_JSON="/home/hyperd/.claude.json"` → `dirname` = `/home/hyperd` (the home directory). This ran AFTER the `aiStackHomeDirTraversal` activation script (`chmod o+x /home/hyperd`) and reset permissions back to 0700 on every rebuild. The three-layer home-dir fix (homeMode, activation script, tmpfiles z rule) was ALL being overridden by this one line in another script. Pattern: **never call `install -d -m` on a directory managed by another NixOS module**. For activation scripts, if the parent directory is a system-managed path (home dir, /var/lib, /etc), omit the `install -d` — it already exists with the correct ownership/mode. Only use `install -d` for subdirectories that genuinely need to be created. Root cause fixed in 1a76021e (nix/modules/roles/cpp-dev.nix line 167).

*Last updated: 2026-06-28 | Maintained by all agents | Seed to RAG after adding patterns*
