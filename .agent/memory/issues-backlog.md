...
[OPEN] ala-flag-flip-blocked-on-enforce-side-asymmetric-verify — The ALA build (slices 1-3) wired only the MINT side: `issue_first_party_leases` returns Ed25519 leases when `CAPABILITY_ASYMMETRIC_LEASE=1`. But the VERIFY side is untouched — `enforce()` verifies every first-party AND candidate lease with `cl.verify(lease, key)` (HMAC, symmetric) at `capability_lease_gate.py:645,690`, and `verify_authoritative` has NO caller in the switchboard. Since `CAPABILITY_LEASE_ENFORCEMENT=1` is already live, flipping `CAPABILITY_ASYMMETRIC_LEASE=1` would mint Ed25519 leases that the HMAC verify rejects (the Ed25519 sig sits in the same `signature` field HMAC reads → bad-signature) → EVERY first-party tool (run_command/file_edit/write_file/…) denied → hard switchboard outage. The build log's Phase-2 ("flip + validate the mint") validated only the mint, missing the enforce-verify interaction. Found by a pre-flip enforce-path read, not by the outage.
  ...
  Action: DO NOT flip the flag until `enforce()` verifies Ed25519 leases via `verify_authoritative(lease, config/aqos/lease-signer-keys.json)` + layers expiry/epoch (OBLIG-1) — a `sig_scheme`-branched admission. That is the C2 scheduler-context issuer rev3 / C2-enforcement scope (rev3 DESIGN drafted, open Qs Q-R3-1/Q-R3-2, NOT yet binding-reviewed or built). This is security-critical admission code and MUST go through the design→review→build cadence, not an inline flip. ALA the MINTER is fully activated + canary-proven (25/25 mint+verify_authoritative-accept); only the routing flag is gated. File: ai-stack/switchboard/capability_lease_gate.py (enforce); C2-SCHEDULER-CONTEXT-ISSUER-DESIGN-20260806.md.
