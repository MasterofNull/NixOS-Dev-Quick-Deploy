...
[OPEN] learning-loop-dormant-local-not-improving — The improve-from-failure loop is not closing: local repeats identical failures within a single session with no learning. Evidence (2026-07-07): training-loop-results.jsonl last modified 2026-05-27 (5+ weeks stale), only 3 runs ever, every run ingest={samples_added:0, dataset_total:0} — ZERO training samples ever captured; training-loop-checkpoint.json is {}. This session local emitted the SAME text-instead-of-tool-call failure 3x (f1-plan-consensus, f2-plan-consensus, f2-session-mode) + truncation (f3) + scaffolding-only (factory-critique). We built RESILIENCE (extract_contribution salvage, never-skip-local) not FIXES — partially violates 'fix the producer'.
  Severity: high
  Root cause: (1) aq-local-training-loop is not scheduled/running; (2) its ingest converts 0 delegation failures to training samples (samples_added:0 is itself a bug); (3) no hook captures the extract_contribution regex-fallback moment (the exact point we detect local emitted text-not-tool-call) as a labeled training pair; (4) F2.2 GBNF constrained-decode is only the cache, not wired into live dispatch requests.
  Action: CLOSE THE LOOP — (a) at the extract_contribution fallback, auto-log the (prompt -> expected tool-call) pair to a training dataset; (b) reactivate + fix aq-local-training-loop ingest (samples_added>0); (c) wire GBNF enforcement into dispatch.py requests (producer-fix for tool-JSON, no retraining); (d) track bench-local-agent before/after to prove improvement. Route through the intent/PRD re-entry round.
  File: scripts/ai/aq-local-training-loop; scripts/ai/lib/round_contribution.py (extract_contribution hook); ai-stack/local-agents/dispatch.py (GBNF wiring); .agents/delegation/training-loop-results.jsonl
