...
[PENDING-CONFIRM 2026-07-02] gemini-tier-3x-model-ids — tiers.google + switchboard aliases + _MODEL_MAP updated from Gemini 2.5 to 3.x per user: default/balanced=gemini-flash-3.5 (medium effort), flagship=gemini-3.1-pro (high). These go through the antigravity wrapper (switchboard→Gemini backend), NOT OpenRouter (unused). CONFIRM before nixos-rebuild: (1) exact api id strings for gemini-flash-3.5 / gemini-3.1-pro, (2) whether effort (low/med/high) is a reasoning_effort request param or encoded in the model id. Assistant training cutoff (Aug 2025) predates Gemini 3.x, so strings are best-effort from user-provided names.
  Severity: low (declarative-only; remote lane pending rebuild regardless)
