...
[FIXED-DEEPENED] local-clone-shallow-breaks-git-groundtruth-tooling — DISCOVERED 2026-08-17 while wiring the PM tracker: the local clone was SHALLOW (`git rev-parse --is-shallow-repository`=true, `.git/shallow` present, only 3 commits reachable from HEAD) even though all commits exist as objects + are pushed to origin (full history remote). Impact: EVERY git-log ground-truth tool silently under-reports — aq-pm-tracker projected 26% (saw 3 commits) instead of 80%; also affects aq-commit-facts, freshness gates, any `git log`-based detection. NOT data loss (work safe on origin). Root cause: shallow clone (likely `--depth` at clone or a re-shallowing fetch). FIX: `git fetch --depth=300 origin main` (deepened to cover all recent work; full `--unshallow` deferred as unnecessary + large for 175+ phases). Action/MONITOR: a plain `git fetch` can re-shallow; if git-log tooling under-reports again, re-deepen. Consider `git fetch --unshallow` once if space allows, or document the shallow-aware depth in the clone/setup.

[FIXED-PENDING-REBUILD] dashboard-compat-gate-missing-psutil — 2026-08-17: focused-ci `dashboard compatibility routes for visibility cards` check FAILs with `ModuleNotFoundError: No module named 'psutil'` whenever a dashboard file is staged (the dashboard backend imports psutil). Same recurring "missing CLI-python dep" class (pydantic/jsonschema/fido2/etc.). Pre-existing — triggered, not caused, by editing dashboard/backend/api/main.py for the ACP deploy. FIX: added `psutil` to `cliPython` (nix/modules/core/base.nix, commit 23097414); lands next rebuild. Until then, dashboard-file commits are blocked by this gate (the ACP main.py router registration is staged in the working tree, lands at the rebuild). Verify post-rebuild: `python3 -c 'import psutil'`.

[FIXED] acp-p0-cross-request-signature-reuse — CRITICAL, found 2026-08-16 by ACP-P1 Antigravity design review (orchestrator-verified), missed by local + orchestrator P0 reviews. ACP-P0's canonical_hash excluded `request_id` (envelope), while the P1 single-use ledger keys on `request_id` and the owner Ed25519 signature is over canonical_hash. So two content-identical requests (same action_manifest/params) with different request_ids share a canonical_hash → one human approval's signature is replayable against a content-identical sibling request that was never itself approved → the same action executes twice on one WebAuthn tap. Root cause: conflated the CONTENT-integrity hash with the AUTHORIZATION/instance binding — the signed material must bind the request instance, not just its content. FIX (root, commit `117aeb66`): added `request_id` to `CANONICAL_FIELDS` in `scripts/ai/lib/approval_request.py` (immutable → no recompute-drift; only `status` stays excluded); golden hash re-pinned ff83ff79→b52bea91; regression test `test_request_id_binding` (content-identical/different-id → different hash; legal status transition → unchanged hash). 8/8 + pytest + tier0 26/0 green. Also drove 4 P1 hardenings (executor-side append-only executed-id ledger, signer UDS rate-limit/max-1-concurrent, software-authenticator build-excluded-from-prod, post-deploy udev/hidraw diagnostic). LESSON: multi-lane independent review (esp. the untrusted advisory lane, verified) catches critical flaws a single reviewer + the author miss — never-go-down/route-to-available paid off. Bounded follow-up to a committed slice, no history rewrite.

[RESOLVED-IN-DESIGN] acp-status-not-authorization-signal — SURFACED 2026-08-16 by ACP-P0 confirmatory review (local Qwen `qopywx`): ACP-P0's `canonical_hash` excludes `status` (correctly — so a legal pending→approved transition doesn't break the executor's tamper check), but `approval_executor.py:123` gates execution on `record["status"] == "approved"`. `status` is therefore not integrity-protected by the hash; in P0 its integrity depends entirely on the (P1) append-only root-owned request store. Risk if left as-is: a forged `pending→approved` flip in a weakly-confined store = forged approval → execution. NOT a P0 code bug (P0 executor's approval is an explicit trusted stub per its docstring; the real store + gate are P1), but a load-bearing design dependency. RESOLVED IN DESIGN (ACP-P1 invariant 6, commit `11a7e478`): P1 makes authorization CRYPTOGRAPHIC — the executor requires a valid owner signature over `canonical_hash` (from the WebAuthn-gated signer) to execute, never `status` alone; `status` becomes a derived reflection of "a valid signature exists". Store confinement remains defense-in-depth. Action: enforce `verify_execution_authorization(record, signature)` in the P1 executor build; confirm no execution path trusts `status` alone. Severity: MEDIUM (design-hardening, pre-build). Good never-skip-local catch.
...
[OPEN][REOPENED — premature-FIXED] antigravity-wake-reports-nudge-not-drain — The two prior [FIXED] entries below (antigravity-auto-wake-path-missing-antigravity-binary, antigravity-inbox-no-auto-trigger-manual-nudge) were marked FIXED on the wrong signal: they validated that the auto-wake FIRES (`method: cli-nudge-ok`, exit 0), NOT that the advisory task actually DRAINS. VALIDATED root cause 2026-08-08: `aq-antigravity-auto-wake.path` IS active(waiting) and DID fire on inbox drops (journal Aug 8 12:13 + 20:04, cli-nudge-ok); the Antigravity IDE process IS running (antigravity-1.23.2 pid 1971139); yet 3 advisories sit pending with EMPTY receipts and no output files written. Mechanism: wake runs `antigravity chat --reuse-window --mode agent <WAKE_PROMPT>` (aq-antigravity-inbox:29) and reports success on the CHAT COMMAND's exit code — the IDE agent does NOT autonomously run the claim→work→complete workflow the prompt asks for. So `cli-nudge-ok` is a FALSE-SUCCESS signal (measures the nudge, not the outcome) — this is exactly the Activation-Gate miss (observable≠functionally-validated) + anti-gaming-adjacent (a green signal for work that didn't happen). Owner correctly flagged the premature "complete".
  ...
  OWNER RE-FLAGGED 2026-09-26 (PRIORITY): during the Stage 1 consensus round, Antigravity's inbox had 3 tasks undrained and the owner had to MANUALLY prompt the IDE again — owner stated this "shouldn't be our default collaborative/delegated structure." DIRECTIVE: auto-engagement must become the DEFAULT for the Antigravity lane; manual `wake --actor owner-manual` is a FALLBACK/exception, not the normal path. This is now a product goal, not just a bug: close the nudge→drain autonomy leg so a dropped advisory reaches receipt.completed + output WITHOUT a human prompt (within the no-API-key / owner-OAuth-IDE constraint — the owner's own session must autonomously run claim→work→complete, e.g. an IDE-side agent config/rule that auto-claims on wake, not just opens a chat). Until closed, Antigravity participates async and the orchestrator must NOT gate a round on it (Rule 18).
...
[OPEN] flat-org-friction-THREE-WAY-CONSENSUS (Claude+Codex+Antigravity, complete 2026-08-15) — All three agents answered the owner's friction question; consensus below. Antigravity's independent view (ANTIGRAVITY-FLAT-ORG-FRICTION-20260808.md) INDEPENDENTLY named the "Nudge-vs-Drain false-success loop" as its #1 friction — the exact bug Claude found+fixed this session (cli-nudge-ok != drain; honest drain-verify, commits ebfef8a7/041878bd) — a strong cross-lane validation. NEW Antigravity-specific frictions Claude+Codex missed: (a) EPISTEMIC-TRUST FEEDBACK VACUUM — advisory lanes receive inbox tasks + write outputs but get NO programmatic feedback on whether their finding was validated/mutated/rejected, so they can't learn or adapt to codebase drift (one-way mirror); (b) REVIEW-GATED AUTH-HASH-DRIFT STALLS — a flagship review vs its authorization doc exhibiting SHA-256 drift stalls the whole lane and the advisory agent can't resolve it (sandbox boundary) — same frozen-design-pin class we hit on C6-B3; (c) EARLY-CRASH SILENCE — wake dispatch crashes before writing a registry row -> silent task loss reported as success. Antigravity REPRIORITIZES: put Liveness & Drain Integrity ABOVE commit-collisions (a silently-failing advisory lane loses half the validation capacity with no alert).
  META-THEME all three converged on: SILENT-FAILURE / FALSE-SUCCESS SIGNALS across the collaboration — nudge!=drain, availability-not-measured, findings-go-stale-unchecked, catch-up-debt-invisible, early-crash-silence. The system reports success/health when work isn't happening. THAT is the highest-order self-improvement target.
  Severity: medium-high (validation-capacity integrity). Frequency: continuous.
  Action: (1) DONE the drain-verify fix (proves the theme is tractable). (2) build a per-finding OUTCOME-FEEDBACK loop so advisory lanes learn if their output was accepted/rejected/superseded (closes the epistemic vacuum + feeds finding-freshness). (3) fold into the structured catch-up-queue work (catchup-queue-unstructured-no-backpressure) — machine-enforced per-entry state + a shared liveness/capacity signal that alerts on ANY silent-failure. (4) the auth-hash-drift stall needs a re-pin/re-auth affordance the gated lane can request, not a hard block.
...
[OPEN] independent-temp-review-mutates-sealed-candidate — During ALA-C2-R1 temp acceptance, the independent reviewer ran `py_compile` inside the sealed candidate and created three unmanifested `__pycache__` files, forcing a correct `REQUEST_REVISION`, evidence-preserving quarantine, and exact reseal before review could continue. Root cause: reviewer instructions prohibited edits but did not mechanically redirect bytecode or make the candidate root read-only.
  ...
  File: temp-first candidate/reviewer orchestration contract; `/tmp/ala-c2-r1-20260808-candidate.contaminated-review1`

[OPEN] disposable-tier0-requires-unmodeled-runtime-evidence-seed — A clean disposable clone could not produce truthful Tier-0 evidence until current runtime-only PULSE/RESUME/issues/registry/candidates projections were copied in, `.agent/qa` was created, and a relevant design artifact was force-staged only in the disposable index. Without those inputs, Phase-0 failed missing operational evidence; staging ignored HANDOFF satisfied cross-surface but violated the artifact-distribution policy. Root cause: the disposable-clone Tier-0 contract does not provide a canonical, validated seeding command that distinguishes runtime evidence from staged release evidence.
  Severity: medium (validation reproducibility; protected repository remained clean)
  Action: add an `aq-tier0-disposable` wrapper that creates the runtime evidence directories, copies only registered operational projections, stages the exact candidate plus an allowed non-runtime design/plan artifact, asserts protected-index isolation, and records the disposable manifest. Never force-stage local-only HANDOFF/PULSE/RESUME artifacts.
  File: `scripts/governance/tier0-validation-gate.sh`; `scripts/testing/harness_qa/phases/phase0.py`; `scripts/testing/test-agent-artifact-policy.py`

[OPEN] local-detached-launch-acknowledgement-gap — Local advisory attempts `local-20260808-121358-f9phj0` and `local-20260808-121407-spnpmc` created slice claims, but neither produced a delegation-registry row, output/progress/heartbeat sidecar, audit evidence, or surviving process. The registry predates both attempts. `delegate-to-local` currently reports `started` as soon as `setsid ... &` returns while the detached child redirects bootstrap stdout/stderr to `/dev/null`; registration occurs later inside the child. A post-fork/pre-registration failure is therefore reported as success and becomes unobservable, and later `--status` misleadingly returns `Task not found`.
  Severity: high (monitoring-first collaboration reliability; advisory work lost, no false review credited)
  Action: add a bounded parent/child registration handshake; report `started` only after registry append; persist deterministic bootstrap stderr at `<output>.bootstrap-error.log`; project claim-backed `unregistered-launch` instead of `not found`. For this occurrence, do not reconcile or cancel nonexistent rows; let the claims expire or release them through their owner, then redispatch once with `--wait` so bootstrap failure is visible.
  File: `scripts/ai/delegate-to-local`; `scripts/ai/lib/dispatch.py`; `.agent/collaboration/slice-claims/local-20260808-121358-f9phj0.claim`; `.agent/collaboration/slice-claims/local-20260808-121407-spnpmc.claim`

[OPEN] c6-b3-live-handoff-and-pre-exec-fence-activation-blocker — Independent exact-commit review of `36a5e2c4` proved the default-ON path is not operable: `DirectRunner.run()` calls `slot_queue.acquire()` without a scheduler context or signer allowlist, so the gate immediately denies `None`; the final epoch reread occurs inside `acquire()` before it returns, leaving a bump-after-acquire/before-`urlopen` race; and the switchboard/live consumer lacks `AQ_REVOCATION_EPOCH_SOCKET_PATH`. The focused test fabricates a context and consumes its epoch bump inside `acquire()`, so it does not exercise the real dispatch seam. CP-1, CP-3, CP-4, and end-to-end A2 fail; default-OFF parity remains PASS and no live regression exists. Further grounding shows the standalone local `DirectRunner` currently has no authenticated scheduler-context producer: C2-SCI contexts are minted inside switchboard tool-admission decisions, while `delegate-to-local`/`dispatch.py` is a separate process path. Passing raw caller JSON would violate the frozen trust model.
  Severity: high (activation-blocking; dormant/default-OFF)
  Action: re-freeze before implementation. Name an authenticated producer-to-dispatch handoff owned by a broker/service boundary (or explicitly make the future host dispatch broker the prerequisite), add a caller-side immediately-pre-provider epoch fence, inject the authority socket into the actual consumer, add reachable-malformed-store and real live-dispatch tests, and resolve CP-4 by a thin client module or an explicit reviewed supersession. Do not activate `CAPABILITY_SCHEDULER_LEASE_GATE` or start B4 implementation until this passes independently.
  File: `scripts/ai/lib/dispatch.py` ~530/586; `scripts/ai/lib/slot_queue.py` ~186/405/454; `nix/modules/services/switchboard.nix`

[OPEN] c6-b3-commit-atomicity-contamination — Commit `36a5e2c4`, described as the C6-B3 amended runtime slice, also contains `.agent/ACTIVATION-AUDIT.md` C2 prose and `.agent/archive/20260807-shell-debris/1nwc`. Those paths are unrelated to the bounded B3 implementation and violate one-slice atomic commit evidence, even though their bytes are inert and no history rewrite is authorized.
  Severity: medium (history/evidence integrity; no runtime impact)
  Action: preserve history under Rule 18, record the contamination in the next B3 release/acceptance evidence, and require explicit pathspec staging plus manifest-vs-commit-diff equality for the corrective commit.
  File: commit `36a5e2c4`

[OPEN] ai-autonomous-improvement-service-fails-deviation-target-unsafe — After `nixos-rebuild switch --flake .#hyperd-ai-dev` (2026-08-08, dirty tree incl. herdr/flake changes), ai-autonomous-improvement.service failed on its timer-fired cycle: PRIMARY `metric-sync-failed`, then it could not write the deviation receipt → `workflow_deviation_io._open_locked` raised `deviation-target-unsafe` (the parent-dir safety check at scripts/ai/lib/workflow_deviation_io.py:48-56 rejects: non-dir / symlink / parent.st_uid != service uid / parent.st_mode & 0o022). `/var/lib/ai-stack` is `root:ai-stack 0750` — likely the deviation receipts subdir is root-owned vs the service's expected uid (Rule 14 ReadWritePaths≠DAC / Rule 13 declarative-ownership class), so the fail-closed safety check denies. Not caused by C6/Foundation-C work (unrelated file path). Masks the upstream metric-sync-failed root cause.
  Severity: medium (a background autonomous loop is down; auto-restart thrashing; no security impact — the safety check is doing its job by denying an unsafe write)
  Action: (1) declaratively chown/verify the deviation receipts dir to the service uid via system.activationScripts(deps=["users"]) or systemd.tmpfiles, same rebuild cycle; (2) separately diagnose the upstream metric-sync-failed (the primary). Reset thrash: `systemctl reset-failed ai-autonomous-improvement.service`. Investigate before next scheduled maintenance.

[RESOLVED 2026-08-08 by commit 3d45e03c — ALA-C2 contract repair, full design→review→owner-auth→acceptance→release; VERIFIED: mint_first_party_leases now emits owner-signed grant_digest (via policy_grant_digest) + policy_revision, the issuer suite mints a REAL lease end-to-end (test-scheduler-context-issuer 68/68 pass), lease-signing PASS] c2-sci-HIGH-ala-lease-schema-mismatch-activation-blocker — Codex Rule-18 catch-up audit (CATCHUP-CODEX-CONFIRMATORY-AUDIT-20260808) minted a REAL lease via lease_signing_authority.mint_first_party_leases() and fed it to the live scheduler_context_issuer.mint_scheduler_context() — it FAILED `lease-fields-malformed`. Root cause: ALA's closed lease construction never emits `grant_digest` or `policy_revision` (scripts/ai/lib/lease_signing_authority.py:68-100), but the C2 issuer REQUIRES both (scripts/ai/lib/scheduler_context_issuer.py:291-325) before its ledger/sign path. The existing C2 tests CONCEAL this by fabricating both fields in fixtures (scripts/testing/test-scheduler-context-ledger.py:257-280). So the shipped default-OFF issuer cannot mint a context from the shipped ALA lease shape — enabling the C6 scheduler fence would deny EVERY such task. Fail-closed, but a complete activation blocker for the claimed ALA→C2-SCI integration. Opus substitute-reviews (B1-B4 PASS) missed it because they trusted the fixtures; genuine Codex end-to-end probe caught it.
  Severity: HIGH (activation-blocking; no live regression — default-OFF)
  Action: define ONE canonical ALA/C2 lease contract — either (a) add owner-authorized SIGNED grant_digest+policy_revision to real ALA leases, or (b) amend C2's required admission tuple + replay key. Add an INTEGRATION test feeding mint_first_party_leases() output directly into mint_scheduler_context() (no hand-augmented fixture). Touches frozen ALA + C2-SCI → needs owner build grant. Blocks c2sci-activate.

[RESOLVED 2026-08-08 by commit 3d45e03c — VERIFIED: issuer now denies on exact `lease_epoch != current_epoch` (was `<`, so future-epoch admission closed); scheduler_context_transport._resolve_epoch rewritten to resolve ONLY through the fail-closed UDS reader (re_lib.resolve_current_epoch), no file/absent→0 fallback remains — the same C6-B3 reader, so the two lanes converged] c2-sci-MEDIUM-epoch-not-deny-closed — Same Codex audit, two epoch defects on the issuer: (1) a valid signed lease stamped revocation_epoch=2 was ACCEPTED while current_epoch=1 (issuer tests only `lease_epoch < current_epoch` at scheduler_context_issuer.py:375-380; the governing design C2-SCHEDULER-CONTEXT-ISSUER-DESIGN-20260806.md:46-58,71-72 requires `!=` deny). This CONFIRMS+UPGRADES the prior "not exploitable prose" note in c2-issuer-b1-review-observations O1 — it IS a real future-epoch admission. (2) scheduler_context_transport.py:238-246 maps an absent/malformed epoch file → 0 and mints on it (fail-OPEN) — the same A2 hole the C6-B3 gate-side reader just closed, still present on the ISSUER side. Partially contained today only by the independent default-OFF downstream C6 fence, but it violates the issuer's own mandatory epoch binding.
  Severity: MEDIUM (fail-open epoch surface on issuer; contained by default-OFF + downstream C6 fence)
  Action: make issuer epoch resolution fail-closed (absent/malformed → deny, mirror revocation_epoch.resolve_current_epoch, never 0) + enforce exact `!=` epoch match with a negative future-epoch vector. Fold with the HIGH under the same C2-SCI activation follow-up. Supersedes O1's "not exploitable" framing.
...
[FIXED] commit-48d92962-swept-5693-files-concurrent-staging-race — Commit 48d92962 ("ALA Phase 1b", intended 4 files) had a 5693-file diff — the concurrent-staging race (a broad `git add`; note `Bash(git add *)` allow-rule in ~/.claude/settings.json:341) folded a large pre-existing tree into the commit's diff, same hazard as `concurrent-orchestrator-commit-authority-race-b3`. NOT pushed (local-only).
  Severity: medium (history hygiene; not destructive, not pushed)
  Action: DONE — coordinated re-commit (backup branch `backup-pre-recommit-20260807` = old tip 749a28bf; done while the concurrent lane was quiesced 8+ min). Built a clean Phase-1b commit (now `7c3b520b`, 4-file diff) from base 5b396cce + only the 4 ALA files, then `git rebase --onto` replayed the 5 descendants. VERIFIED: `git diff --name-only HEAD backup` = 0 (byte-identical working state → ZERO data loss); the ~5689 extra files trace to ancestor 539ec969 and remain in history (never lost). New tip e8e876e2; all commit hashes downstream of 5b396cce changed (concurrent lane was dormant, so no divergence). Backup branch retained until owner confirms, then deletable. Structural fix still OPEN: aq-slice-claim + commit lock so lanes never `git add -A` into each other's commits (router phase-2, links [concurrent-orchestrator-commit-authority-race-b3]).

[FIXED] antigravity-auto-wake-path-missing-antigravity-binary — The antigravity-auto-wake user service (built ae3bff1d, enabled + rebuilt) fires correctly on an inbox drop (systemd.user.path works), but the wake INVOCATION failed `method: cli-nudge-binary-missing`: aq-antigravity-inbox's WAKE_ARGV runs `antigravity chat …` by bare name via PATH, and the systemd USER service PATH omits the system profile (`/run/current-system/sw/bin`, where `antigravity` lives). So the auto-nudge never reached the IDE. Found live on the first real auto-wake (C2 rev4 review drop).
  Severity: medium (auto-wake fires but no-ops; owner-manual wake still works)
  Action: DONE + VALIDATED LIVE 2026-08-07 (rebuilt; test wake on the pending C2 drop = ok:True method:cli-nudge-ok exit_code:0 — the auto-wake lane now fully self-triggers, no manual nudge). wakeScript now `export PATH="/run/current-system/sw/bin:/run/wrappers/bin:$PATH"` before calling aq-antigravity-inbox, so `antigravity` resolves. File: nix/modules/services/antigravity-auto-wake.nix. Until the next rebuild, antigravity contributes to a round only via a manual `aq-antigravity-inbox wake <task> --actor owner-manual`.

[FIXED] antigravity-inbox-no-auto-trigger-manual-nudge — The Antigravity advisory lane dropped tasks into a watched inbox, but the IDE never polls the dir, so every delegation required the owner to manually prompt the IDE to run `aq-antigravity-inbox next/claim`. The active trigger `aq-antigravity-inbox wake` (runs `antigravity chat --reuse-window --mode agent`) existed but was exposed only as a CLI, wired to no watcher; and its `--actor` is intentionally gated to `owner-manual` (it puppets the owner's OAuth IDE session — no autonomous principal may fire it).
  Severity: medium (workflow friction; blocked hands-off multi-agent rounds)
  Action: DONE — NEW default-OFF `nix/modules/services/antigravity-auto-wake.nix`: a per-USER systemd path unit (runs in the owner's own session, as the owner) watches the inbox dir and fires `aq-antigravity-inbox wake --actor owner-manual` on each new drop — the owner's session automating the owner's own step, so the actor gate stays honest and the harness never drives the IDE from a service principal. Oneshot self-gates on a pending-eligible task (claim/receipt/lane-state churn filtered), rate-limited (6/60s). Flag `mySystem.aiStack.antigravityAutoWake.enable`. File: nix/modules/services/antigravity-auto-wake.nix; corrects ROUTING-FIX-PROPOSAL.md's wrong "the IDE watches the dir" assumption.

[RESOLVED — flip live + validated 2026-08-07] ala-flag-flip-blocked-on-enforce-side-asymmetric-verify — DONE. enforce-side asymmetric verifier BUILT + independently reviewed + N3-validated (a4c496ec) THEN FLIPPED LIVE (86ec204e, owner-rebuilt): switchboard env CAPABILITY_ASYMMETRIC_LEASE=1. End-to-end validated: switchboard issue_first_party_leases → 25 ed25519 leases via the confined authority, 25/25 verify_authoritative-accepted; N3 full-enforce()-path 25/25 admit; dashboard asymmetric_lease=on/1 active key/ok; no fail-closed. Private key stays 0400 in the authority (verify != forge). ALA activation COMPLETE (Phase 0/1/1b/2). REVERT = remove 2 env lines + rebuild → HMAC. Build log ENFORCE-ASYMMETRIC-VERIFY-BUILD-LOG-20260807.md. ORIGINAL: The ALA build (slices 1-3) wired only the MINT side: `issue_first_party_leases` returns Ed25519 leases when `CAPABILITY_ASYMMETRIC_LEASE=1`. But the VERIFY side is untouched — `enforce()` verifies every first-party AND candidate lease with `cl.verify(lease, key)` (HMAC, symmetric) at `capability_lease_gate.py:645,690`, and `verify_authoritative` has NO caller in the switchboard. Since `CAPABILITY_LEASE_ENFORCEMENT=1` is already live, flipping `CAPABILITY_ASYMMETRIC_LEASE=1` would mint Ed25519 leases that the HMAC verify rejects (the Ed25519 sig sits in the same `signature` field HMAC reads → bad-signature) → EVERY first-party tool (run_command/file_edit/write_file/…) denied → hard switchboard outage. The build log's Phase-2 ("flip + validate the mint") validated only the mint, missing the enforce-verify interaction. Found by a pre-flip enforce-path read, not by the outage.
  Severity: high (blocks activation; flipping = outage)
  Action: DO NOT flip the flag until `enforce()` verifies Ed25519 leases via `verify_authoritative(lease, config/aqos/lease-signer-keys.json)` + layers expiry/epoch (OBLIG-1) — a `sig_scheme`-branched admission. This is its OWN slice `ENFORCE-ASYMMETRIC-VERIFY-DESIGN-20260807.md` (drafted 2026-08-07, open Qs Q-E1..Q-E4, routed for binding review), DISTINCT from the C2 scheduler-context issuer rev3 (that mints C6 scheduler-context tokens; it does not touch enforce()'s first-party admission — earlier "C2-enforcement" wording conflated them). Security-critical admission code → design→review→build, not an inline flip. ALA the MINTER is fully activated + canary-proven (25/25 mint+verify_authoritative-accept); only the routing flag is gated. File: ai-stack/switchboard/capability_lease_gate.py (enforce ~:690); ENFORCE-ASYMMETRIC-VERIFY-DESIGN-20260807.md.

[FIXED] ala-socket-no-client-access-would-outage-phase2 — ALA slice-2 wired the confined signer's socket to the authority's OWN group (0660 aq-lease-signing-authority:aq-lease-signing-authority), but the sole client (the switchboard) runs as the human primaryUser, which is NOT in that group. Flipping CAPABILITY_ASYMMETRIC_LEASE=1 would have made every switchboard→authority connect fail → `_request_asymmetric_first_party_leases` fail-closes to {} → ALL first-party leases denied → hard outage. The build log's "0660 group-restricted" implied a shared client group that was never created. Found by a pre-Phase-2 deploy-context check (socket group vs switchboard uid), not by the outage.
  Severity: high (guaranteed Phase-2 outage)
  Action: DONE — mirrored the execution-cell-runner client-group pattern: NEW group aq-lease-signing-clients (primaryUser + authority both members); serve() chgrps the socket to AQ_LEASE_SIGNING_CLIENT_GROUP after bind (fail-closed + loud on error, never widens); tmpfiles dir 0750→0755 (matches effective RuntimeDirectory mode). Grants connect-only, NOT key access (private key stays 0400 user-only); authority is not an oracle (mints manifest set only), so a group peer gains nothing forge-worthy. File: scripts/ai/lib/lease_signing_authority.py (serve); nix/modules/services/lease-signing-authority.nix.

[FIXED] ala-epoch-file-missing-readonlypaths-unit-fail — `config/capability-lease-epoch` did not exist, yet the confined `aq-lease-signing-authority` service lists it in `ReadOnlyPaths`; a non-existent `ReadOnlyPaths` target fails the systemd unit at start, so ALA Phase-1 activation would have brought the service up FAILED. Found by a WR-3-style deploy-context preflight before the activation rebuild (not by the rebuild itself). Root cause: the epoch SSOT was implicit — both readers (gate `capability_lease_gate.py:114`, authority `lease_signing_authority.py:48`) tolerated the missing file via a 0-floor fallback, so nothing forced the file to exist until a confined service bind-mounted it.
  Severity: medium (would fail activation, not silent corruption)
  Action: DONE — created `config/capability-lease-epoch` = `0` (genesis; C6 epoch-bump increments it). Parity-verified: both readers resolved 0 before AND after (missing-file fallback was already 0), so the HMAC Phase-1 path is byte-unchanged. File: config/capability-lease-epoch; nix/modules/services/lease-signing-authority.nix:107.
...
Severity: high
2026-08-08 recurrence: `config/refactor-milestones.json` still describes C3-C6 as not started and `assets/aqos-progress-tracker.html` hardcodes Foundation C as `notstarted/0%` with Q1/Q2 pending, despite the committed Foundation C work and current C6-B3 REQUEST_REVISION. The tracker must not call rejected B3 shipped or accepted, but it must truthfully project partial Foundation C progress, the ALA-C2 activation blocker, and the TEG design gate.
Action: prepare an independently reviewed tracker amendment that derives Q1/Q2, Foundation B2, and Foundation C status from ratification/acceptance/review records, updates `config/refactor-milestones.json`, the generated visible projection, and focused tests atomically, and preserves the governing-versus-operational provenance distinction. Stop manually maintaining HTML `tracks`, `gateRows`, counts, and provenance arrays; generate them from `aq-refactor-status`.
...
- **2026-08-08 recurrence**: `aq-antigravity-auto-wake.path` was active and its service exited `0/SUCCESS` with `method: cli-nudge-ok`, yet the inbox still held two eligible unclaimed tasks (`c2-scheduler-context-issuer-rev4-review.md` and `ala-c2-c6-b3r-advisory-20260808.md`). A successful IDE nudge therefore does not prove claim, execution, or receipt progress.
...
## [DONE 2026-08-02] Runner hardening breaks execution-cell-adapter hermetic Service Coverage — RESOLVED (adapter test _start_runner RunnerConfig gets allow_self_bind=True; full 49/49 Service Coverage green; production Nix/runner defaults UNCHANGED). Integrated by orchestrator on codex handoff.
- **status**: IN-FLIGHT — production runner remains default-OFF; no deployment or live traffic occurred. Tracker semantic rebuild focused tests pass, but final Tier-0 acceptance is blocked.
- **finding**: isolated Tier-0 reaches Phase-0 check `0.10.44`, where `test-execution-cell-adapter.py` now passes 46/49 and reports `runner-unreachable`. The accepted runner hardening correctly changed `RunnerConfig.allow_self_bind` to default `False`, while the hermetic adapter test constructs `RunnerConfig` without explicitly opting into development self-bind.
- **root cause**: cross-slice test-fixture contract drift. Production default-off hardening was validated in the runner suite, but the existing adapter integration fixture implicitly depended on the former self-binding default and was not included in the runner acceptance matrix.
- **severity**: HIGH pre-release integration regression; safely caught before commit, rebuild, socket activation, or traffic.
- **action**: prepare and independently review a one-file amendment that sets `allow_self_bind=True` only in `_start_runner()` inside `scripts/testing/test-execution-cell-adapter.py`; rerun its 49 checks outside the managed sandbox and isolated Tier-0. Never relax the production Nix or runner defaults. Then re-pin the tracker issue-source digest and complete acceptance.
- **file**: `scripts/testing/test-execution-cell-adapter.py:194`; `ai-stack/switchboard/execution_cell_runner.py:553`; Phase-0 `0.10.44`

## [FIXED 2026-08-03] Runner deploy-bug #7: RuntimeDirectory re-groups the socket -> clients can't connect
- **status**: FIXED (pending rebuild re-validation) — found by the R6 Step-1 deploy-exercise (adapter OFF, no live impact).
- **root cause**: the socket ListenStream (/run/aq-execution-cell-runner/control.sock) lived INSIDE the service RuntimeDirectory (RuntimeDirectory=aq-execution-cell-runner). systemd applies the service User ownership to that dir AND the socket-unit-created socket inside it on service start, clobbering SocketGroup=aq-execution-cell-clients -> aq-execution-cell-runner. After the first activation-triggering connect, clients (switchboard) get PermissionError. Confirmed by elimination: runner adopts fd 3 (allow_self_bind=0 + it started+accepted = bug #6 passed), so only systemd re-grouped the file.
- **not caught earlier**: offline acceptance can't exercise real systemd RuntimeDirectory management; the R6 incremental deploy-exercise is exactly what surfaced it.
...
## [FIXED 2026-08-05] R7 deploy-bug #13: git absent from runner PATH -> FAILURE_QUARANTINED at cell-create
- **status**: FIXED (pending rebuild) — the ACTUAL cause of the R7 quarantine (my earlier tree-not-proven diagnosis was WRONG; the real stage is cell-create, tree_proven=None).
- **root cause**: R2 create_cell (execution_cell_clone.py:628) AND the out-of-cell validator (execution_cell_validator.py:194) invoke git via BARE subprocess.run(["git",...]) — R2's reviewed contract is "the runner provides git on PATH". The runner service PATH was coreutils/findutils/grep/sed/systemd only — NO git. Every clone raised FileNotFoundError -> caught by create_cell's outer handler -> the catch-all FAILURE_QUARANTINED (code "quarantined") at STAGE_CELL_CREATE. Looked like a cell quarantine; was "git not found". Mirror-sync worked only via absolute pkgs.git/bin/git.
...
## [RESOLVED 2026-08-06] R7 GREEN achieved; bug #12 fence diagnosis was a MISATTRIBUTION
- **outcome**: R7 confinement spine reached typed GREEN end-to-end (receipt 5da042fb / 590b698a, stage=final-fence, tree_proven=True). Isolation proven (real README.md untouched). Clears C4 runner-live-cell gate.
- **#12 correction**: the "false-quarantine at the supervise fence" diagnosis was WRONG. Every quarantine was stage=cell-create (never the fence). The actual cascade, each fixed live: #13 git-not-on-PATH -> create_cell catch-all FAILURE_QUARANTINED; then isolation-violation (17 committed absolute data/*.json symlinks -> untracked, owner path A); then path-escape (BY DESIGN — R2 does not materialize not-yet-existing write targets; GREEN demo must target an EXISTING tracked file). The supervise fence works (tree_proven=True). _log_unproven_tree + StandardError=journal are retained permanent observability (never fired — the fence is healthy).
- **root-cause tooling fix**: added scripts/testing/cell-submit.py — a reusable argparse CLI so the cell adapter is exercised with plain args (single sg -c quote layer), eliminating the triple-nested `sg -c "... python3 -c \"...\""` that mangled newlines/special chars. Also: use `git commit -F <file>` (not inline -m with arrows/`${}`), and Write+cat-append for multi-line docs (not inline heredocs with `${}` -> zsh bad-substitution).
- **open observability follow-up (unchanged)**: runner discards TypedFailure.detail on cell-create failure — log its low-cardinality reason class so isolation-violation vs base-oid-unreachable vs path-escape detail is visible without repo spelunking.

## [OPEN 2026-08-08] autonomous-improvement-metric-sync-false-green
- **severity**: HIGH.
- **scope**: hourly `ai-autonomous-improvement.service` and the recursive-learning health contract.
- **evidence**: the host timer is active and the service fires hourly, but every inspected cycle logs `Metric sync failed: no such table: routing_log`, then prints `No improvement cycle needed - system healthy` and exits `0/SUCCESS`. The live and repository `routing_metrics.db` schemas expose `routing_decisions`, not `routing_log`.
- **root cause**: `trend_database.py` consumes an obsolete routing schema, while `autonomous_loop.py` collapses metric-collection failure and the legitimate no-trigger state into the same `None` result. The system therefore suppresses the anomaly that should trigger its own repair loop.
- **action**: implement a bounded fail-closed repair: resolve the canonical routing database, normalize supported schema versions, emit a typed metric-source failure, return non-zero on observation failure, route the typed deviation into PRSI, and add service-coverage tests/dashboard state proving `failed`, `healthy-no-trigger`, and `cycle-completed` are distinct. Do not auto-apply a repair from incomplete telemetry.
- **file**: `ai-stack/autonomous-improvement/trend_database.py`; `ai-stack/autonomous-improvement/autonomous_loop.py`; `scripts/automation/prsi-orchestrator.py`; `nix/modules/services/autonomous-improvement.nix`.

## [OPEN 2026-08-08] canonical-workflow-deviations-not-recursively-ingested
- **severity**: HIGH.
- **scope**: WORKFLOW-CANON deviations across Codex, Claude, Gemini/Antigravity, local inference, validation, review, and release lanes.
- **evidence**: `aq-loop` records incomplete iterations in its private state and appends an issue only after exhausting all retries; PRSI separately consumes delegation failures and `aq-report` actions. There is no shared typed deviation contract or immediate deduplicated intake for early review, validation-environment, authorization, subject-drift, false-start, or monitoring failures. The owner therefore still has to notice and re-trigger recurring collaboration failures.
- **root cause**: deviation detection, issue logging, learning-candidate generation, and remediation scheduling are separate producers without one event contract, idempotency key, lifecycle owner, or receipt chain. Workflow events can be visible to one projection while remaining dark to the recursive loop.
- **action**: add `aq.workflow-deviation.v1` and a single intake path implementing `detect -> preserve evidence -> deduplicate -> issue upsert -> learning candidate -> bounded shadow remediation -> independent verification -> promote or reject`. Low-risk documentation/diagnostic corrections may proceed automatically; mutations of runtime authority, security policy, deployment, secrets, release state, or external systems remain approval-gated. Every state must project to QA and the command-center dashboard.
- **file**: `scripts/ai/aq-loop`; `scripts/automation/prsi-orchestrator.py`; `.agents/events/a2a-events.jsonl`; `config/schemas/`; dashboard PRSI/progress projections.

## [OPEN 2026-08-08] workflow-event-writer-unavailable-in-managed-agent-sandbox
- **severity**: MEDIUM.
- **scope**: mandatory RESUME/PULSE event updates during managed Codex execution.
- **evidence**: `scripts/ai/aq-event resume` failed with `EROFS` opening `.agents/events/a2a-events.jsonl`, because the current managed environment exposes `.agents` read-only even though the repository workflow mandates event emission at phase boundaries.
- **recurrence**: monitored Antigravity review `antigravity-20260808-145322-jv6fry` failed before usable registration, and its subsequent `--status` call raised `EROFS` while `_ensure_dirs()` attempted to touch `.agents/delegation/registry.jsonl`. A route cannot be called agentic or monitored when both dispatch and observation require direct writes unavailable to the caller.
- **root cause**: the canonical event writer has no brokered write path when direct filesystem mutation is denied, and the CLI does not park or spool the event for a host-side writer.
- **action**: route event emission through the connection-reliability broker or a narrow append-only host service; return a durable receipt before claiming the checkpoint succeeded. Until then, report the failed checkpoint explicitly and preserve the intended event in the issue ledger rather than silently continuing.
- **file**: `scripts/ai/aq-event`; `scripts/ai/lib/event_log.py`; agent connection broker/event projector configuration.
[OPEN] workflow-deviation-recovery — Tier-0 reports the active plan directory has no tracker.json projection.
  Severity: medium
  Action: Add the bounded C2 progress-tracker/dashboard projection after C1 brokered ingestion; keep C1A atomic.
...
[DONE] capability-intake-candidate-count-brittleness — Adding the valid Herdr H0 candidate failed the existing registry test because it asserted a magic total of 14 candidates rather than the canonical identity set.
  Severity: medium
  Action: Replace the count with an explicit unique candidate-ID set and add deny-by-default Herdr assertions so additions/removals produce actionable identity evidence.
  File: scripts/testing/test-capability-intake.py ~test_registry_schema

[DONE] capability-intake-fuzzy-test-stale-cache — The capability-intake regression reused a repository `.cache` verdict, hiding that the production auditor no longer enforced its asserted one-edit denylist typo defense.
  Severity: high
  Action: Give every run a fresh TemporaryDirectory-backed cache and restore a bounded distance-one check for denylisted names of at least six characters; exact names retain the exact denial reason.
  File: scripts/testing/test-capability-intake.py ~test_auditor_fuzzy; ai-stack/mcp-servers/shared/tool_security_auditor.py ~_evaluate

[DONE] herdr-h0-invalid-report-frontmatter — The H0 intake artifact used unsupported `doc_type: report`; the canonical frontmatter schema permits plan artifacts in `.agents/plans/`.
  Severity: low
  Action: Reclassify the intake report as `doc_type: plan` without changing its body or PREPARED_ONLY semantics.
  File: .agents/plans/herdr-agent-operations/H0-INTAKE-REPORT.md ~frontmatter

[OPEN] herdr-agent-operations-missing-progress-projection — Tier-0 reports the new Herdr program directory has no tracker.json, so H0-H4 status does not yet project into the default user-facing progress tracker.
  Severity: medium
  Action: Add the canonical tracker projection in H1 together with default-OFF package/config state; do not widen the accepted H0 intake commit.
  File: .agents/plans/herdr-agent-operations ~tracker.json
[IN-FLIGHT] HERDR — H0 incorrectly recorded pinned v0.7.5 as Apache-2.0 — Direct inspection of the immutable source shows LICENSE, Cargo.toml, README and nix/package.nix consistently declare AGPL-3.0-or-later with a commercial-license option; corrected H0/H1 records and blocked implementation pending owner license acceptance.
  Severity: high
  Action: independently re-review corrected H0/H1 provenance, then record owner acceptance of AGPL obligations or a commercial-license record before H1 implementation.
  File: .agents/plans/herdr-agent-operations/H0-INTAKE-REPORT.md
[DONE 2026-08-08] HERDR-DOC-METADATA — The first Herdr provenance/design commit attempt was blocked because the new operations runbook omitted mandatory Owner and Last Updated fields — Normal commit hooks failed closed; added the required metadata, reran the metadata gate, and requested an exact metadata-only independent re-review. Existing enforcement is sufficient to prevent recurrence.
  Severity: low
  Action: none; preserve normal hooks and never bypass check-doc-metadata-standards.sh.
  File: docs/operations/herdr-agent-operations.md
[DONE 2026-08-08] HERDR-FRONTMATTER-STATUS — The second Herdr design commit attempt was blocked because H1 used unsupported plan status `prepared`; an earlier ad-hoc frontmatter command masked its failure with `|| true` — Changed the status to schema-valid `draft`, retained tracker-derived FROZEN state, and removed failure-masking from the final validation path.
  Severity: low
  Action: never suffix mandatory validation with `|| true`; rely on the frontmatter schema and normal hooks.
  File: .agents/plans/herdr-agent-operations/H1-DESIGN-PACKET.md
[OPEN] NETWORK-PROFILE-INTEROPERABILITY — During the Herdr design cycle the host switched Wi-Fi networks and connectivity projected `connected (site only)` — Read-only diagnosis found no current auth/secret, DHCP, resolver-service, VPN, or outbound-firewall failure. The likely cause is the universal `10-wifi-reliable-dns` dispatcher replacing per-link DNS and route-only `~.` on every Wi-Fi up/change, which is intentionally fail-closed for bad DHCP DNS but incompatible with some captive-portal, enterprise split-DNS, and local-only networks. No security control was relaxed.
  Severity: medium
...
[OPEN] PM-TRACKER-IFRAME-OPAQUE-ORIGIN-CORS — `dashboard.html` embeds `assets/aqos-progress-tracker.html` in `<iframe sandbox="allow-scripts">` (deliberately, no `allow-same-origin`, per an existing test). Now that the tracker page live-fetches `GET /api/pm/progress`, a sandboxed frame without `allow-same-origin` has an OPAQUE origin, so the embedded copy's `fetch()` sends `Origin: null` and is cross-origin even against the same host/port; `dashboard/backend/api/main.py`'s `CORSMiddleware` only allows an explicit origin list (no `"null"`), so the browser will likely withhold the response from JS in the embedded panel (request still reaches the server; only the response read is blocked). Degrades gracefully to the page's built-in error state + Retry button — not a crash — but the embedded mini-view may never go live without a fix. The full-page route (`/assets/aqos-progress-tracker.html`, linked from the Program tab as "Open full-page tracker") is unaffected — same-origin navigation, no CORS involved.
  Severity: medium
  Action: decide and implement one of: (a) allow `Origin: null` on `/api/pm/progress` specifically (narrow CORS carve-out, security review needed), or (b) drop the iframe's own fetch and have the parent `dashboard.js` fetch once and `postMessage` the data into the sandboxed frame. Out of scope for the pm-dashboard-live build (build record: `.agents/plans/program-progress-tracker/PM-DASHBOARD-LIVE-BUILD-20260817.md`).
  File: dashboard/backend/api/main.py ~CORSMiddleware; dashboard.html ~iframe#panel-program; assets/aqos-progress-tracker.html ~load()

[OPEN] heartbeat-prefill-invisibility-wedge-vs-slow — The local agent-loop heartbeat/progress instruments (dispatch.py heartbeat: child_idle_s/no_progress_timeout; agent_executor.py progress: llm_stream_chunks/llm_waiting) expose completed-step logic + stream-chunk count + idle time, but NOT in-flight LLM prefill progress. During a large-prompt prefill on the APU (e.g. a ~5-6K-token prompt after the agent reads a 449-line/17KB file), the instruments show `llm_waiting`, 0 chunks, child_idle climbing — BYTE-IDENTICAL to a genuine wedge. This ambiguity directly caused a slow-but-working task to be misread as wedged and cancelled (the exact "it's not responding, I'll do it myself" anti-pattern the owner flagged).
  Severity: medium
  Root cause: streaming instrument only increments on first decoded token; prefill phase is a blind spot.
  Action: surface in-flight prefill state — an `awaiting_first_token` bool, `prefill_elapsed_s`, and last-known tok/s — in progress.json + heartbeat, so an operator can distinguish "prefilling, first token pending Ns" from "stuck." Emit in ai-stack/local-agents/agent_executor.py (streaming loop) + scripts/ai/lib/dispatch.py (heartbeat merge).
  File: ai-stack/local-agents/agent_executor.py; scripts/ai/lib/dispatch.py

[OPEN] codebase-context-misses-shebang-scripts — The `codebase-context` Qdrant collection (25,980 pts) does not surface `scripts/ai/` shebang executables (no .py extension) — verified: a semantic query + a payload-path filter for `aq-role-route` returned only thematically-similar .py files (route_aliases.py), never the target script. So embed-retrieval over codebase-context alone cannot front-load the span for the large class of backlog tasks that edit scripts/ai/ shell/shebang tools.
  Severity: medium
  Root cause: the codebase indexer's file-glob likely restricts to recognized code extensions; extension-less executables are skipped.
...
[OPEN] read-file-gate-char-guard-env-var-misnamed — agent_executor.py reads the prefill char guard as `int(os.getenv("n", "24000"))` — the env var is literally named "n". Harmless (defaults to 24000) but wrong: it can't be tuned via a sensible name and risks accidental override by a stray "n" env var.
  Severity: low
  Action: rename to LLAMA_MAX_PROMPT_CHARS (or the intended name). File: ai-stack/local-agents/agent_executor.py

[OPEN] local-read-loop-defeats-context-gate — With micro grounding (budget cleared) + assembler (spans front-loaded) + read_file gate (whole-file reads summarized), local STILL fails bounded edits: it calls read_file repeatedly (6x on aq-role-route, never edit_file) and hits repeated-read stagnation. The gate traded "prompt-too-large" for "re-read-loop." Root cause: local's behavior is to read the FULL file before editing, but a 17KB file cannot fit the APU's ~24000-char prefill budget with overhead — so it MUST edit from partial context (front-loaded spans or ranged reads), but it doesn't; it re-reads the same summary hoping for the full file → identical result → stagnation abort. The assembler already front-loaded the exact target function (_matches_exclude at 241-280) yet local ignored it.
  Severity: high (this is THE blocker for local completing bounded edits on non-tiny files)
  Fix options (ranked): (a) strengthen gate note to explicitly forbid re-read + direct edit [DONE, unproven]; (b) ESCALATING gate — on a repeat read of the same file, return a DIFFERENT message (breaks identical-result stagnation) that hard-directs edit_file + names the front-loaded citations; (c) on _MAX_READS_WITHOUT_EDIT, inject the assembler's front-loaded spans + "edit now" instead of aborting; (d) deeper: prompt/tune local to edit from partial context.
  File: ai-stack/local-agents/agent_executor.py (_READ_FILE_GATE_NOTE, read-stagnation/nudge logic ~L1639/1910)

[OPEN] local-edit-from-summary-fails-oldstring-match — SHARPENS local-read-loop-defeats-context-gate. Confirmed root cause of local's ~50% edit reliability: local calls edit_file against the GATED SUMMARY content (outline+chunks), but edit_file's old_string must match the file EXACTLY — the summary is excerpted/reformatted, so old_string mismatches, the edit fails, local re-reads to get exact content, gets the same summary, retries, and hits repeated-read stagnation. Measured: same task (exclude-granularity) landed a correct edit on one run (used the front-loaded EXACT spans) and stagnated on another (re-read the summary). Non-deterministic.
  Severity: high (the core local reliability blocker for edits on non-tiny files)
  Deterministic fix: ESCALATING gate — on a REPEAT read of the same file within a task, return the assembler's EXACT front-loaded spans (verbatim code, from the .context.json sidecar) + "here is the exact code; call edit_file with this exact text; do NOT read again." Gives local exact old_string material + breaks the identical-result stagnation. File: ai-stack/local-agents/agent_executor.py (read gate + read-stagnation path).

[OPEN] local-diffs-plausible-but-incorrect — Local's "edit-captured" dogfood diffs are NOT reliably correct. dogfood-01 (exclude-granularity) produced a confident diff that (a) left the matching logic functionally UNCHANGED and (b) referenced a helper `_lane_sub_entity_matches_exclude` that does not exist (invented in a comment). A separate manual run of the SAME task produced a genuinely correct fix. So local's output quality is variable AND independent of "did an edit land" — edit-captured != correct.
  Severity: high (defines the review burden — every local diff needs remote verification; ~half of "successes" may be wrong)
  Implication: the remote-review-before-commit guardrail is load-bearing, not optional. Do NOT auto-trust local diffs. Morning review: verify each dogfood-diffs/*.patch against the actual issue; expect a meaningful reject rate.
  File: .agents/delegation/dogfood-diffs/ (review target)

[OPEN] local-autonomous-edit-completion-rate-~5pct-EMPIRICAL — DEFINITIVE dogfood result (2026-08-18): 20 task-runs (12 bounded backlog tasks + 8 retries), framework fully active (assembler + read_file gate + micro grounding + poll-for-edit runner). Outcome: 1/20 landed an edit, and that one (dogfood-01) is functionally WRONG (unchanged logic + invented helper). So local's autonomous bounded-edit completion = ~5% edit-landed / ~0% correct. Most tasks churned the full 50-min poll cap with no edit. Framework PLUMBING works (0 prompt-too-large, honest detection, zero false-commits); local CAPABILITY on multi-step edit tasks is the ceiling.
  Severity: high (redefines strategy — local NOT ready for autonomous task completion)
  Recommended next: (a) do NOT run autonomous local grinds expecting usable output; (b) close the capability gap first — escalating gate serving EXACT spans + a post-edit VERIFY loop (re-read edited region, confirm valid + intent-matching) so wrong/no-op edits self-reject; (c) narrower single-line decomposition OR route real edits to a more capable lane while local does prep/research/review. Owner's "prove capability with data" directive answered: the data says not yet.
  File: .agents/delegation/dogfood-overnight-summary.md

[OPEN] local-reread-stagnation-despite-verbatim-frontload — DEFINITIVE: verbatim front-loading confirmed reaching local (exact indented code present in prompt, assembler fired tokens=1813), yet local still re-reads the file 4x and gets stagnation-aborted with 0 edits. So the blocker is BEHAVIORAL + NON-DETERMINISTIC: local sometimes edits from front-loaded code (direct test succeeded, correct edit 22min) and sometimes ignores it and re-reads to abort (runner: 4-5 consecutive stagnation failures). Success rate ~1 in 5-6.
  Severity: high (the recurring killer of local autonomous edits)
  Fix (recommended, high-leverage): convert the repeated-read STAGNATION ABORT into an EDIT-FORCING intervention — when local would be aborted for re-reading, instead inject one corrective turn: "STOP reading; the exact code is already in your context under '## Relevant prior knowledge'; call edit_file NOW using that exact text." Give one forced edit attempt before any abort. File: ai-stack/local-agents/agent_executor.py (repeated-read stagnation handling, ~L1639/1910).

[FIXED] local-agent-tool-call-json-artifact-leak — The local agent's streaming/GBNF tool-call parser leaks the tool-call envelope's closing punctuation into the LAST argument's value (e.g. run_command receives "grep ... file\n},\n"). The shell-safety guard (_SHELL_CONTROL_PATTERN) then correctly rejects the trailing \n as a metacharacter, the model retries the same artifact-laden call, and after 5 rejections the tool-failure-stagnation guard aborts the task (calls=13, never reached an edit). This is the 4th distinct local-agent failure mode found; blocks tasks where local explores via run_command before editing.
...
[OPEN] codex-review-local-agent-batch-20260821 — Independent Codex review of this session's local-agent+harness batch (cc63ac57..4650b1e6) found 12 defects Claude missed; full report .agent/collaboration/codex-review-local-agent-batch-20260821.md. HIGH: (1/2) write_region path-traversal + non-atomic write (no workspace-allowlist/symlink reject); (3) cassette request_key COLLIDES distinct requests (excludes model id, thinking-mode, freq/repeat penalty, stop, tool metadata) — can mask regressions; (4) strict replay silently falls back to LIVE (invalid mode->off, missing path->live, import fail->live) — contradicts 'replay=no network'; (5) agent loop has NO hard termination bound (max_tool_calls ignored, while True — alternating changing outputs evades every stagnation counter); (6) GBNF still permits malformed JSON (unescaped control chars/bare newlines; _object_rule ignores required + unparenthesized unknown-type alternations break nested objects). MEDIUM: (7) artifact-strip corrupts legit commands (printf '%s' ']' -> broken) — fix at JSON-parser boundary not command tail; (8) retry paths drop task_type -> change inference profile + cassette identity; (9) write_region no stale-line-drift guard + non-atomic + forces newline; (10) new tests not in tier0/pytest gate; (11) cassette may expose model content (no 0600/redaction/path-constraint). PROCESS: (12) commit-scope + co-author-trailer contract violations.
  Severity: high (multiple security-adjacent; shipped as 'done' without independent review — the governance gap the owner flagged)
  Action: tier-triage the fixes through the redesigned workflow; Tier-0 ones (path traversal, grammar-constrains-output, replay-masks-regression) first. Do NOT bulk-fire fixes without owner-visible tier declaration.

[OPEN] antigravity-autonomous-lane-nonfunctional — VERIFIED 2026-08-21: the Antigravity/Gemini lane produces NOTHING in autonomous mode despite `aq-antigravity-inbox wake` returning cli-nudge-ok. Inbox status: 9 pending, 0 claimed, 0 drained; tasks stale up to ~5.3 days (teg-c1-sliceone 455579s; 5 ACP reviews ~372-424k s each). The wake DELIVERS a nudge but the passive IDE never claims/acts on the inbox — so we've been counting a lane that contributes zero autonomously while appearing 'engaged'. This is a concrete 'reported-working-but-isnt' automation gap.
  Severity: high (a whole review/consensus lane is dead weight in autonomous mode; masks the true independent-review coverage)
  Root cause (suspected): IDE does not poll/claim the inbox; the auto-wake watcher (antigravityAutoWake) is default-OFF and even when nudged the IDE requires the owner to be present + manually act. So 'autonomous Antigravity' is a misnomer — it's owner-manual-only.
  Action: (1) treat Antigravity as owner-manual-ONLY in the workflow (do NOT count it toward autonomous independent-review coverage); (2) either make the inbox→IDE claim actually work (investigate the IDE-side watcher/extension) or formally reclassify the lane; (3) drain the 9-task backlog with owner present or archive stale ones. Feeds the workflow-redesign meta-prompt (lane-reality vs lane-assumption).
  File: scripts/ai/aq-antigravity-inbox; nix antigravityAutoWake watcher; IDE-side integration

  ROOT CAUSE CONFIRMED 2026-08-21: (a) auto-wake watcher service is INACTIVE despite ai-dev.nix `antigravityAutoWake.enable = true` (declarative-vs-runtime gap — service never activated/crashed); (b) `wake` shells out to `antigravity chat --reuse-window` (WAKE_ARGV) — depends on the interactive IDE being open + a human/IDE actually acting, NOT headless; (c) after owner IDE-nudge the pending count dropped 9->3 but NO new output files were produced — tasks CLEARED/aged, not COMPLETED. So the lane is architecturally IDE-interactive, not autonomous. Fix: restart/repair the auto-wake service AND accept Antigravity is owner-manual/IDE-present-only — never count it toward autonomous coverage.

[OPEN] antigravity-review-verified-criticals-20260821 — Antigravity (untrusted-advisory lane) independent review of the local-agent batch found 2 CRITICALs Codex+Claude BOTH missed; BOTH VERIFIED TRUE by orchestrator. (1) CRITICAL replay-executes-tools-LIVE: aq-replay-bench registers REAL tools (register_shell_tools/register_file_tools) + runs executor._execute_with_tools (line 197) — the cassette stubs only _call_llama (the LLM), NOT the tool registry, so replaying a cassette EXECUTES its run_command/write_file/write_region calls LIVE on the host. The 'safe offline A/B' premise is false; a cassette with mutating/destructive tool calls pollutes the workspace on replay. Fix: --mock-tools/--dry-run flag stubbing the tool registry in aq-replay-bench (+ default it ON for bench). (2) CRITICAL cassette-record-not-process-safe: llm_cassette.py:193 opens append mode with no fcntl.flock — concurrent writes corrupt the JSONL (real for multi-agent/parallel; we run sequential now). Fix: flock LOCK_EX on append. ALSO confirmed Codex findings + added specifics: write_region EOF no-trailing-newline merges lines (import osimport sys); retry block catches ReplayMiss and masks it with a mis-parameterized second call; commit-contract Step-8 evidence-block omissions.
  Severity: high (CRITICAL #1 makes the benchmark harness unsafe by default)
...
[DONE 2026-08-23] local-retry-turn-exhausts-prefill-budget — Dogfood task `local-20260823-185103-t0q2v8` reached the model under micro grounding and the 1,500-token assembler cap, but its first turn was prose-only. The one-shot no-action/GBNF repair path replayed the full failed response into subsequent messages; repair and prose synthesis then failed closed at 25,062/25,065 chars against the unchanged 24,000-char guard. Runtime ended after 1,805 seconds with zero tool calls and zero edits.
  Severity: high
...
[IN-FLIGHT] local-dogfood-late-invalid-edit-after-time-budget — The same measured task crossed its 1,800-second budget, required explicit registry cancellation, and left a late one-file edit without a completion/test receipt. The edit asserted ordering on a `set`, failed the focused test immediately, and was removed without adoption. This confirms the retry can now reach an edit but not reliable semantics or bounded completion.
  ...
  Follow-up evidence correction: `local-20260824-065315-siyc4b` was launched with both `AGENT_WALL_CLOCK_SECS=900` and `AQ_AGENT_WALL_BUDGET_S=900`, then registry-cancelled only 19.5 seconds after creation. A tool-layer wall-time report incorrectly suggested a multi-hour overrun; the persisted task timestamps prove the 900-second cap was never exercised. The real defect is cancellation topology: SIGTERM targeted supervisor PID 760904 while separately sessionized `aq-agent-loop` child PID 760927 survived temporarily, yet registry state was published as cancelled without death confirmation. No terminal progress receipt or model edit reached the worktree.
  Forward-slice evidence: cancellation lifecycle F1 exact subject `3eb80ef46ca0c5044e06dd3ba260f9a4ac721517ef194aa51382d5a5c2eb8d14` received independent PASS after proving whole-group TERM/KILL/death confirmation, wall-boundary reconciliation, typed exactly-once terminal receipts, and registry append/update/terminal serialization. The remaining acceptance step is a fresh measured one-file dogfood run inside the explicit wall budget.
  Action: integrate the accepted cancellation lifecycle, then run one controlled one-file task with the explicit wall override and independently validate any edit; do not resume the uncurated overnight queue until that task completes correctly inside budget.
  File: scripts/ai/lib/dispatch.py; scripts/ai/lib/task_registry.py; scripts/testing/test-local-delegation-artifact.py
...
[CLOSED-AS-MISDIAGNOSIS 2026-08-24] model-swap-qwen36-thinking-not-suppressed — Re-investigated against the LIVE server (active.gguf = Qwen3.6-35B-A3B-UD-Q5_K_S.gguf, llama-server pid 2614832, --jinja flag present). Could NOT reproduce "enable_thinking:false still returns <think>" through the real code path. 10 live probes at :8080, covering exactly what agent_executor.py sends:
  - Nested `chat_template_kwargs:{"enable_thinking":false}` via /v1/chat/completions: clean (no <think>, no reasoning_content) — plain prompt, complex/adversarial multi-file-bug prompt, AND GBNF grammar-constrained tool call (tool_grammar.tool_call_grammar output), both non-streaming AND streaming (SSE delta reconstruction: content='Paris', reasoning=''). This is the exact payload build_llama_payload(task_type="agent") produces.
  - Baseline (no chat_template_kwargs at all): thinking IS on by default for Qwen3.6 (reasoning_content fills, content='') — confirms the model's default really did flip with the swap, so the concern was legitimate to raise.
  - Reproduced the ORIGINAL failure signature (empty content, reasoning_content eats the budget) ONLY by deliberately using `enable_thinking` at the TOP LEVEL of the payload instead of nested in chat_template_kwargs — this is the exact pre-existing pitfall already documented in llm_config.py's own docstring (lines 9-13, written for the Qwen3-35B swap). Also reproduced a bounded (harmless) `<think>\n\n</think>\n\nParis` via the raw /completion endpoint with a hand-built prompt, which doesn't apply chat_template_kwargs at all (that endpoint doesn't render the Jinja template from JSON fields — a naive /completion probe would look "broken" even when the real /v1/chat/completions path is fine).
  Conclusion: the CURRENT llm_config.py SSOT (chat_template_kwargs nested enable_thinking:false) already correctly suppresses thinking on Qwen3.6-35B-A3B-UD-Q5_K_S in every mode the real agent loop uses. The original diagnostic probe most likely used one of the two known-bad shapes above (top-level key, or bare /completion) rather than the SSOT-built payload, and mis-attributed a pre-existing pitfall to a new Qwen3.6 regression. No code change made — test-local-inference-l2b.py already passes 16/16 unmodified (golden hash untouched, no re-pin needed) and test-tool-call-grammar.py passes 16/16 unmodified.
  Severity: downgraded high→informational. If a real live agent task still exhibits wall_clock_exceeded-with-zero-tool-calls after this, re-open with the EXACT captured outbound payload (not a hand-built probe) — cache_prompt=true prefix-reuse across differing enable_thinking values, or APU contention (was compounding the original report), remain untested alternate causes.
  File: ai-stack/mcp-servers/shared/llm_config.py (build_llama_payload — unchanged); ai-stack/local-agents/agent_executor.py (confirmed call sites at lines 2832/2870 already build via SSOT, already hit /v1/chat/completions not /completion)

[OPEN] pytest-asyncio-missing-blocks-harness-suites — `pytest_asyncio` is not importable in the active python3 env (ModuleNotFoundError). Every `async def test_*` in scripts/testing/{test-reread-intervention,test-noaction-intervention,test-edit-feedback,test-write-region,test-agent-loop-bounds}.py fails/errors as a result (pytest has no asyncio plugin registered — "Unknown config option: asyncio_mode" warning, then the coroutine is never awaited/collected). Confirmed pre-existing: reproduced on a clean checkout with zero code changes. Sync-only suites (test-tool-call-grammar.py, test-local-inference-l2b.py) pass 16/16 and 16/16 respectively, so this is purely a test-collection/env gap, not a functional regression.
  Severity: medium (false-negative test signal — these 5 suites currently report ~30 FAILED but the underlying code is untested, not broken; anyone trusting the red X get a false failure-not-found-here result)
  Action: declare pytest-asyncio in the nix-managed python env (whichever derivation provides scripts/testing's python3 — check flake python packages / nix/pkgs for the test env) so `python3 -m pytest scripts/testing/` actually exercises the async intervention/harness suites instead of erroring out before they run.
  File: scripts/testing/test-reread-intervention.py; test-noaction-intervention.py; test-edit-feedback.py; test-write-region.py; test-agent-loop-bounds.py; nix python env declaring the testing toolchain

[OPEN] cancellation-lifecycle-slice-regressed-inference — The cancellation-lifecycle F1 slice (commit 93f1eff4, dispatch.py + task_registry.py +378 lines: whole-group TERM/KILL/death-confirmation + child watchdog/wall-clock rewire) REGRESSED local agent inference: after it, the agent-loop model_call produced 0 tokens for the full wall budget (1501s acceptance run + 600s isolation run, both silent) while a direct curl to the same model/endpoint returns in 8s. A/B PROVEN: reverting ONLY dispatch.py+task_registry.py to 93f1eff4^ restored inference immediately (tool_calls=3, llm_streaming within 1 min). Something in the slice's child sessionization / watchdog / streaming-read wiring blocks the first token. REVERTED to un-block all local work.
  Severity: high (blocked ALL local edit work)
  Action: RE-DO the cancellation improvement (the clean-termination goal is real + was validated) but VERIFY agent-loop inference produces tokens after — likely the sessionization (new PGID/session for the child) or the watchdog cap breaks the child's HTTP stream to llama.cpp. Add an inference-smoke check to the slice's acceptance. File: scripts/ai/lib/dispatch.py; scripts/ai/lib/task_registry.py

[MILESTONE-2026-08-25] local-first-clean-correct-edit-in-budget — MEASURED clean run (VSCodium closed, load ~6.5 = mostly the AI stack's own services): local autonomously produced a CORRECT bounded edit on dogfood-01 (exclude-granularity) in 4 tool calls, edit on step 4 at 31 min, within the 40-min wall budget. The edit is genuinely correct (sound logic, parses, pure string ops — no phantom refs/imports; saved .agents/delegation/dogfood-diffs/dogfood-01-MEASURED-CORRECT-20260825.patch). This CLEARS Codex's gate ('one one-file task completes correctly inside budget'). The accumulated fix stack (restored inference + grammar + context-supply-chain + write_region + interventions + edit-feedback) works end-to-end. THROUGHPUT REALITY: ~8-11 min/LLM step even clean -> needs a >=40min budget for a multi-step edit (a 30min budget fails). MINOR gaps still open: (a) task ends 'failed' because local doesn't cleanly signal completion after editing (hits wall budget post-edit) — the edit is delivered but the completion-signal is missing; (b) the cancellation-lifecycle redo is still queued (its revert 1c317f2e restored inference).

[FIXED] dogfood-payload-budget-cap-miscalibrated — The overnight dogfood queue produced 0/25 edits: EVERY task failed in 0.4s with "dogfood payload budget exceeded before HTTP: 21896 Unicode JSON chars > 14000". Cause: the runner set AQ_LOCAL_DOGFOOD_BUDGET=1, whose _DOGFOOD_PAYLOAD_JSON_LIMIT=14000 (agent_executor.py:158) is BELOW both the real payload (~21896 chars with the full context supply chain) AND the model's own LLAMA_MAX_PROMPT_CHARS=24000 prompt guard — so it rejected valid payloads pre-HTTP. The validated measured run (first clean correct edit, 2026-08-25) used NO dogfood budget.
  Severity: high (silently zeroed an entire overnight batch — the payload grew past 14000 when the assembler context supply chain was added; the cap never tracked it)
  Fix (DONE): (1) removed AQ_LOCAL_DOGFOOD_BUDGET from the runner ENV to match the validated measured-run config; (2) producer fix — raised _DOGFOOD_PAYLOAD_JSON_LIMIT 14000->32000 (payload JSON is legitimately larger than the 24000 prompt guard). File: scripts/ai/aq-local-dogfood-run; ai-stack/local-agents/agent_executor.py
...
[RESOLVED 2026-08-30] session-start-machine-mode-contract-drift — FIXED: added `--machine` flag to scripts/ai/aq-session-start emitting a closed JSON object (status:"closed" + task/skills/hints/lessons/resume/system-state), human output suppressed via `_human_printf` wrapper; unknown flags still exit 2; regression test scripts/testing/test-aq-session-start-machine.sh. Codex-implemented, Claude-reviewed+validated (live JSON smoke OK, human output intact). ORIG — Lights-Out guidance requires machine-mode CLI interactions, but `aq-session-start --task ... --machine` rejects `--machine` as an unknown argument. Operators must currently use human-formatted output for this mandatory session entrypoint.
  Severity: medium
...
[OPEN] shallow-dogfood-verify-cmd-false-passes-deletions — HIGH. The scaffolded dogfood-01 rerun (2026-08-29, scaffold 605018be) got local to LAND its first edit, but the behavioral verify gate FALSE-PASSED a destructive edit. dogfood-01's verify_cmd is `{file} --help >/dev/null 2>&1`; local's edit deleted `route()` (scripts/ai/aq-role-route:292) and `_emit()` (:281) while adding the intended `_matches_exclude` change, yet `aq-role-route --help` still exits 0 because argparse prints help and returns BEFORE `route()` is ever called. A smoke test that does not exercise the deleted surface cannot catch a deletion, so the coach never fired on a broken edit and the runner captured it as "edit-captured". Nothing reached the repo (capture-only + revert; safety held).
  Severity: high (a false-passing gate is the dangerous failure mode — it would wave destructive edits through if the loop ever auto-committed)
  Action: (1) strengthen dogfood-01 verify_cmd to exercise `route()` (e.g. `python3 -c "import ast,sys; src=open('{file}').read(); ast.parse(src); assert 'def route(' in src and 'def _emit(' in src"` or a real bounded route invocation), not just `--help`. (2) Better: add a generic post-edit guard in `_verify_edit_quality` — "did this edit delete a top-level def/class that is still referenced elsewhere in the file?" — so ANY destructive edit is coached regardless of the per-task verify_cmd. Connects to edit-verify dead-code/no-op checks (they check ADDED-but-unused; this is the inverse: DELETED-but-still-referenced).
  File: scripts/ai/aq-local-dogfood-run (dogfood-01 val_task verify_cmd); ai-stack/local-agents/agent_executor.py (_verify_edit_quality)

[OPEN] local-edit-destructive-span-correct-logic-wrong-blast-radius — HIGH. Same run: local's edit CONTENT was logically correct (a reasonable, well-documented implementation of the exclude-lane-granularity backlog item — `if "-" in token: return False` to stop a specific sub-agent token excluding a whole lane), but the edit's BLAST RADIUS was destructive: the edit_file old_string/new_string spanned ~96 lines (270-365) and replaced them with 28, deleting `_emit()` + `route()`. This is the measured local edit-mechanics limitation: local can now produce correct LOGIC when scaffolded with an honest exact target, but its edit_file span selection still over-reaches and silently drops surrounding code. Capability-graduated stewardship (Rule 21), not shame — the scaffold WORKED (first landed edit, correct core logic); the next scaffold layer is narrowing edit blast radius.
  Severity: high (blocks promotion of local landed-edits to auto-review until blast radius is controlled)
  Action: scaffold local toward minimal-span edits — e.g. instruct/enforce that old_string be the smallest unique span around the change (not a whole-function-and-neighbors block), and add the post-edit deleted-def guard above as the safety net. Re-run scaffolded dogfood-01 after to measure whether local produces a NON-destructive correct edit.
  File: ai-stack/local-agents/agent_executor.py (edit path + _verify_edit_quality); scripts/ai/aq-local-dogfood-run (prompt guidance)

[ACCEPTED-LIMITATION] destructive-deletion-guard-classbody-order-limitation — The destructive-deletion coaching guard (_find_destructive_deletion) intentionally does NOT model order-sensitive class-body LOAD_NAME. A shape like `class C:\n    value = route()\n    route = staticmethod(lambda: 1)` raises NameError at runtime (the call precedes the class-local rebinding) but symtable reports `route` as class-locally assigned, so the guard returns None (false negative). Surfaced by Codex independent review round 4.
  Severity: low (benign, backstopped)
  Action: ACCEPTED as proportionate — this is a coaching heuristic for local-agent edits, not a soundness-complete analyzer; a local agent will essentially never emit this shape, modeling per-statement class-body execution order is disproportionate, and the miss is backstopped (the edit never auto-commits; the compile + behavioral verify checks run regardless). Documented in-code at _find_destructive_deletion and pinned by test case (r). Revisit only if a real class-body destructive edit is ever observed.
  File: ai-stack/local-agents/agent_executor.py (_find_destructive_deletion)

[RESOLVED 2026-08-30 a2465867 — sync --wait dispatch, validated live 11/12] nightly-training-loop-tasks-never-register-in-service-context — HIGH. The nightly `ai-local-training-loop.service` (systemd oneshot, daily 04:15 timer, User=svcUser) has been producing ZERO useful output since ~2026-08-27. Root cause (diagnosed 2026-08-29): every eval task it submits via `delegate-to-local` (background mode) NEVER appears in `.agents/delegation/registry.jsonl`, so after idle=2401s (the 40-min `--task-timeout`) the loop marks it `STALE: missing registry entry` → FAIL score=None. All 12 eval-pack tasks fail this way, so one run wastes ~8h (12×40min) producing nothing. PROVEN context-specific: an identical `delegate-to-local` background submission run manually as `hyperd` registers correctly within seconds (registry 1074→1075, task present); the training loop's tasks (04:27–09:07, e.g. local-20260829-042741-2la68t) have 0 registry hits AND no `.agents/delegation/outputs/<id>.log` — the detached background dispatch dies before registering OR writing anything. So the break is the SERVICE EXECUTION CONTEXT, not the registration code. CORRECTION: earlier this was mis-logged as "orphans/hangs" and two runs were reaped as "hung" — they were NOT hung, they were this broken run legitimately grinding one failing task per 40 min. RESOLUTION (a2465867): the loop now dispatches synchronously via `delegate-to-local --wait` (foreground survives the service context; setsid was absent from the service path), and `TimeoutStartSec` was changed `infinity` → `"64800"` (18h finite safety net — a legit run is ~8h + retries, so it stays generous but bounds a true hang). Validated live: 11/12 pass, real scores, improvement proposal generated. [Historical note: while broken, `infinity` was the deployed value and a legit-but-failing run genuinely took 8h+.]
  Severity: high (a core nightly self-improvement automation is fully broken + silently wasting 8h/night)
  Prime suspects: (a) the service's restricted `path = [bash coreutils curl git python3]` is missing a binary the background dispatch needs (works under hyperd's full PATH); (b) a `Type=oneshot` service + `delegate-to-local`'s detached/backgrounded child interaction — the escaped child fails or is reaped in the service cgroup/sandbox before `registry.append`.
  Action (Rule 19 — fix the producer): reproduce delegate-to-local background dispatch AS svcUser under the service sandbox, capture the crash. Likely fix options: (1) have the training loop dispatch synchronously (`--wait`) or in-process (like the dogfood runner, which works) instead of detach+poll; or (2) complete the service `path`/environment so the background dispatch survives. Also add a finite-but-generous `TimeoutStartSec` (~= worst-case 12 tasks × 40min × retry margin, e.g. 18h) as a safety net for a TRUE hang — NOT a short one (would kill legit long runs).
  File: nix/modules/services/mcp-servers.nix:1924 (ai-local-training-loop service); scripts/ai/aq-local-training-loop:500 (_submit_task); scripts/ai/lib/dispatch.py (background register path)

[DONE] aqos-ai-fit-catalog-eligibility-bypass — Independent review found that the P0 evaluator selected Vulkan for unsupported vendors or after CUDA was disabled, bypassing catalog `vendor_ids`, `stable`, and `auto_select`; zero/unknown per-model VRAM requirements could also authorize full offload.
  Severity: high
