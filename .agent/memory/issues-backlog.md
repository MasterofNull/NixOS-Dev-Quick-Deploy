...
[OPEN] nightly-training-loop-tasks-never-register-in-service-context — HIGH. The nightly `ai-local-training-loop.service` (systemd oneshot, daily 04:15 timer, User=svcUser) has been producing ZERO useful output since ~2026-08-27. Root cause (diagnosed 2026-08-29): every eval task it submits via `delegate-to-local` (background mode) NEVER appears in `.agents/delegation/registry.jsonl`, so after idle=2401s (the 40-min `--task-timeout`) the loop marks it `STALE: missing registry entry` → FAIL score=None. All 12 eval-pack tasks fail this way, so one run wastes ~8h (12×40min) producing nothing. PROVEN context-specific: an identical `delegate-to-local` background submission run manually as `hyperd` registers correctly within seconds (registry 1074→1075, task present); the training loop's tasks (04:27–09:07, e.g. local-20260829-042741-2la68t) have 0 registry hits AND no `.agents/delegation/outputs/<id>.log` — the detached background dispatch dies before registering OR writing anything. So the break is the SERVICE EXECUTION CONTEXT, not the registration code. CORRECTION: earlier this was mis-logged as "orphans/hangs" and two runs were reaped as "hung" — they were NOT hung, they were this broken run legitimately grinding one failing task per 40 min. `TimeoutStartSec=infinity` is intentional (a legit run is genuinely 8h+).
